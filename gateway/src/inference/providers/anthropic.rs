...
/// Prepares messages for the Anthropic API by ensuring they meet API requirements:
/// 1. First message must be from User role
/// 2. Last message cannot be from Assistant role
fn prepare_messages(messages: Vec<AnthropicMessage>) -> Result<Vec<AnthropicMessage>, Error> {
    let mut prepared_messages = messages;

    // Anthropic requires that there is at least one message and it is a User message.
    // If it's not we will prepend a default User message.
    match prepared_messages.first() {
    ...
    if let Some(last_message) = prepared_messages.last() {
        if last_message.role == AnthropicRole::Assistant {
            prepared_messages.push(AnthropicMessage {
                role: AnthropicRole::User,
                content: vec![AnthropicMessageContent::Text {
                    text: "[listening]",
                }],
...
mod tests {
    ...
    fn test_initialize_anthropic_request_body() {
        ...
        // Test case 1: Basic request with consecutive user messages
        ...
        let messages = vec![
            AnthropicMessage {
                role: AnthropicRole::User,
                content: vec![AnthropicMessageContent::Text { text: "test_user" }],
            },
            AnthropicMessage {
                role: AnthropicRole::User,
                content: vec![AnthropicMessageContent::Text { text: "test_user2" }],
            },
            AnthropicMessage {
                role: AnthropicRole::Assistant,
                content: vec![AnthropicMessageContent::Text {
        ...
        assert_eq!(
            AnthropicRequestBody::new("claude", &inference_request).unwrap(),
        ...
        assert!(AnthropicRequestBody::new("claude", &inference_request).is_err());

        // Test case 3: JSON mode with function type JSON
        let inference_request = ModelInferenceRequest {
    ...
    fn test_consecutive_messages() {
        ...
        let consecutive_user_messages = vec![
            AnthropicMessage {
                role: AnthropicRole::User,
        ...
        let result = prepare_messages(consecutive_user_messages.clone()).unwrap();
        assert_eq!(
            result, consecutive_user_messages,
            "Consecutive user messages should be preserved"
        );

        // Test consecutive assistant messages
        let consecutive_assistant_messages = vec![
            AnthropicMessage {
                role: AnthropicRole::User,
                content: vec![AnthropicMessageContent::Text {
                    text: "What's 1+1?",
            ...
            AnthropicMessage {
                role: AnthropicRole::Assistant,
                content: vec![AnthropicMessageContent::Text { text: "1+1 is" }],
            },
            AnthropicMessage {
                role: AnthropicRole::Assistant,
                content: vec![AnthropicMessageContent::Text { text: "2" }],
            },
        ];
        let result = prepare_messages(consecutive_assistant_messages.clone()).unwrap();
        let mut expected = consecutive_assistant_messages;
        expected.push(AnthropicMessage {
            role: AnthropicRole::User,
            content: vec![AnthropicMessageContent::Text {
                text: "[listening]",
            }],
        ...
        assert_eq!(
            result, expected,
            "Consecutive assistant messages should be preserved with final user message"
        );
    }
