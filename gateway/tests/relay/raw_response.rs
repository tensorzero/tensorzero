...
async fn test_relay_raw_response_error_non_streaming() {
    // Use empty configs - the relay will forward to downstream using shorthand syntax
    let downstream_config = "";
    let relay_config = "";

    let env = start_relay_test_environment(downstream_config, relay_config).await;

    // Use shorthand provider::model syntax that relay forwards to downstream
...
/// Test that relay passes through raw_response from downstream errors (streaming request)
...
/// response includes raw_response from downstream.
#[tokio::test]
async fn test_relay_raw_response_error_streaming() {
    ...
    // Should be an error status (502 Bad Gateway)
    assert!(
        !response.status().is_success(),
        "Response should be an error status"
    );
    ...
    let raw_response = body
        .get("raw_response")
        .expect("Relay streaming error should pass through raw_response from downstream");
    assert!(raw_response.is_array(), "raw_response should be an array");
...
// Note: Streaming raw_response passthrough is tested by the existing
