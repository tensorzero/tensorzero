...
def test_simple_query_chat_function(embedded_sync_client: TensorZeroGateway):
    ...
    for inference in inferences:
        ...
        assert isinstance(output_0, MessageContentText)
    ...
    for inference in inferences:
        ...
        for message in messages:
            ...
            for content in message.content:
                ...
                if content.type == "tool_call":
                    ...
                    assert isinstance(content, MessageContentToolResult)
                    assert content.id is not None
                    assert content.name is not None
        ...
        for output_item in output:
            if output_item.type == "text":
                assert isinstance(output_item, MessageContentText)
                assert output_item.text is not None
            elif output_item.type == "tool_call":
                assert isinstance(output_item, InferenceResponseToolCall)
