...
@pytest.mark.asyncio
async def test_async_raw_usage_non_streaming(async_openai_client):
    ...
    assert result.usage.tensorzero_raw_usage is not None, "tensorzero_raw_usage should not be None"
    assert isinstance(result.usage.tensorzero_raw_usage, list), "tensorzero_raw_usage should be a list"
    assert len(result.usage.tensorzero_raw_usage) > 0, "tensorzero_raw_usage should have at least one entry"

    # Verify structure of first entry
    entry = result.usage.tensorzero_raw_usage[0]
...
@pytest.mark.asyncio
async def test_async_raw_usage_not_requested(async_openai_client):
...
@pytest.mark.asyncio
async def test_async_raw_usage_streaming(async_openai_client):
    ...
    async for chunk in stream:
        ...
        if chunk.usage is not None:
            ...
            if raw_usage is not None:
                ...
                assert len(raw_usage) > 0, "tensorzero_raw_usage should have at least one entry"
