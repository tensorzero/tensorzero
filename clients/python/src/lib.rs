...
use python_helpers::{deserialize_from_pyobj, is_not_required, parse_datapoint, parse_dynamic_evaluation_run_episode_response,
...
impl BaseTensorZeroGateway {
    ...
    fn prepare_inference_params(
        ...
        let cache_options: Option<CacheParamsOptions> = if let Some(cache_options) = cache_options {
            if is_not_required(py, cache_options) {
                return Err(PyValueError::new_err(
                    "NotRequired fields should not be treated as Optional. Please provide a value or omit the field.",
                ));
            }
        ...
        let output_schema: Option<serde_json::Value> = if let Some(output_schema) = output_schema {
            if is_not_required(py, output_schema) {
                return Err(PyValueError::new_err(
                    "NotRequired fields should not be treated as Optional. Please provide a value or omit the field.",
                ));
            }
        ...
        let extra_body: UnfilteredInferenceExtraBody = if let Some(extra_body) = extra_body {
            if is_not_required(py, extra_body) {
                return Err(PyValueError::new_err(
                    "NotRequired fields should not be treated as Optional. Please provide a value or omit the field.",
                ));
            }
        ...
        let extra_headers: UnfilteredInferenceExtraHeaders =
            if let Some(extra_headers) = extra_headers {
                if is_not_required(py, extra_headers) {
                    return Err(PyValueError::new_err(
                        "NotRequired fields should not be treated as Optional. Please provide a value or omit the field.",
                    ));
                }
