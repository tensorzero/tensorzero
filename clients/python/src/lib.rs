...
impl TensorZeroGateway {
    ...
    #[pyo3(signature = (*, ids, output_source="inference"))]
    fn get_inferences(
        this: PyRef<'_, Self>,
        ids: Vec<Bound<'_, PyAny>>,
        output_source: &str,
        ...
        let fut = client.get_inferences(ids, output_source);
        let response =
            tokio_block_on_without_gil(this.py(), fut).map_err(|e| convert_error(this.py(), e))?;
        convert_response_to_python(this.py(), response, "tensorzero", "GetInferencesResponse")
    }
...
impl AsyncTensorZeroGateway {
    ...
    fn get_inferences<'a>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(this.py(), async move {
            let res = client.get_inferences(ids, output_source).await;
            Python::attach(|py| match res {
                Ok(response) => {
                    convert_response_to_python(py, response, "tensorzero", "GetInferencesResponse")
                }
                Err(e) => Err(convert_error(py, e)),
            })
        })
