#!/usr/bin/env python3
"""
Standalone test for DatapointMetadataUpdate with UNSET sentinel value.

This test doesn't import the full package to avoid import errors.
"""

import json
from dataclasses import asdict, dataclass
from tensorzero.generated_types import DatapointMetadataUpdate, UNSET, _UnsetType
...
def verify_generated_code():
    """Verify the actual generated code has UNSET."""
    print("Test 6: Verify generated code")
    print("-" * 70)

    try:
        with open('/Users/shuyangli/Developer/tensorzero/clients/python/tensorzero/generated_types.py') as f:
            content = f.read()

        # Check for DatapointMetadataUpdate with UNSET
        if 'class DatapointMetadataUpdate:' in content:
            # Find the class definition
            start = content.find('class DatapointMetadataUpdate:')
            end = content.find('\nclass ', start + 1)
            if end == -1:
                end = len(content)
            class_def = content[start:end]

            if '| _UnsetType = UNSET' in class_def:
                print("✓ Generated DatapointMetadataUpdate has UNSET default")
            else:
                print("✗ Generated DatapointMetadataUpdate does NOT have UNSET default")
                print(f"  Found:\n{class_def[:200]}")
        else:
            print("✗ DatapointMetadataUpdate not found in generated types")
    except Exception as e:
        print(f"⚠ Could not verify generated code: {e}")
