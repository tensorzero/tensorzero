...
from typing import Any, Generic, Optional, TypeVar, cast  # noqa: E402

from pydantic import BaseModel  # noqa: E402

# New generics for typed Agent
TIdentifier = TypeVar("TIdentifier", bound=str)
TSystemModel = TypeVar("TSystemModel", bound=BaseModel)
TUserModel = TypeVar("TUserModel", bound=BaseModel)

try:
    from agents import Agent as _BaseAgent  # noqa: F401
    from agents import Runner as _BaseRunner
...
if _AGENT_SDK_AVAILABLE:
    ...
    class TensorZeroAgent(Generic[TIdentifier, TSystemModel, TUserModel], _BaseAgent):
        """Agent subclass that validates TensorZero prompt arguments.

        Use :meth:`from_identifier` to obtain an instance bound to a specific
        TensorZero function / variant.  The resulting object exposes strict
        validation of *system* and *user* prompt-argument dictionaries against
        the Pydantic models compiled by ``tensorzero-gen``.
        """

        _identifier: TIdentifier  # full model name (tensorzero::…)
        _system_model: type[BaseModel]
        _user_model: type[BaseModel]
        ...
        @classmethod
        def from_identifier(
            cls, ident: TIdentifier, *, generated_pkg: str = "generated"
        ) -> "TensorZeroAgent[TIdentifier, Any, Any]":
            """Factory that infers the correct arg models from the active TZ context."""

            tz_ctx = _tensorzero_context.get()
            if tz_ctx is None:
                raise TensorZeroAgentsError(
                    "TensorZeroAgent must be constructed inside a 'with_tensorzero_agents_patched' context"
                )

            prompt_tpl = tz_ctx.function_variant_to_prompt_templates.get(str(ident))
            if prompt_tpl is None:
                raise TensorZeroAgentsError(
                    f"Identifier not present in config: {ident}"
                )

            def _load(schema_path: Optional[str]) -> type[BaseModel]:
                if not schema_path:
                    return BaseModel
                import re

                module_name = re.sub(
                    r"\W|^(?=\d)", "_", Path(schema_path).stem + "_model"
                )
                try:
                    mod = import_module(f"{generated_pkg}.schemas.{module_name}")
                    return cast(type[BaseModel], getattr(mod, "Model"))
                except (ModuleNotFoundError, AttributeError):
                    # Fallback to BaseModel – validation will be a no-op
                    return BaseModel

            system_model = _load(prompt_tpl.system_schema)
            user_model = _load(prompt_tpl.user_schema)

            return cls(
                model=str(ident), _system_model=system_model, _user_model=user_model
            )  # type: ignore[arg-type]

        # Runner will handle validation; TensorZeroAgent keeps only construction helpers

    class TensorZeroRunner(Generic[TIdentifier, TSystemModel, TUserModel], _BaseRunner):
        """Runner subclass that performs argument validation/injection."""
        ...
        def for_identifier(
            ...
            ident: TIdentifier,
            *,
            generated_pkg: str = "generated",
        ) -> "TensorZeroRunner[TIdentifier, Any, Any]":
            """Return a *typed* runner instance pre-configured for *ident*.

            The generic parameters are filled with the concrete Pydantic models so
            static type checkers can verify the *system* / *user* kwargs.
            """

            agent = TensorZeroAgent.from_identifier(ident, generated_pkg=generated_pkg)
            # Build runner and *pretend* it is specialised – this is safe and gives
            # editors the correct types via cast.
            return cast(
                "TensorZeroRunner[TIdentifier, Any, Any]",
                cls(),  # new instance with default _BaseRunner init
            )
        ...
        async def run(
            self,
            agent: "TensorZeroAgent[TIdentifier, TSystemModel, TUserModel] | str",
            *messages: Any,
            system: TSystemModel | dict | None = None,
            user: TUserModel | dict | None = None,
            **kwargs: Any,
        ) -> Any:  # noqa: ANN401
            # If a string/identifier was passed, build an agent on the fly
            if not isinstance(agent, TensorZeroAgent):
                agent = TensorZeroAgent.from_identifier(agent)  # type: ignore[arg-type]

            def _convert(arg: Any, model: type[BaseModel]) -> dict[str, Any]:
                if arg is None or model is BaseModel:
                    return {}
                if isinstance(arg, model):
                    return arg.model_dump()
                return model.model_validate(arg).model_dump()

            sys_args = _convert(system, agent._system_model)  # type: ignore[attr-defined]
            user_args = _convert(user, agent._user_model)  # type: ignore[attr-defined]

            meta = kwargs.setdefault("metadata", {})
            for k, v in {**sys_args, **user_args}.items():
                meta[f"tensorzero::arguments::{k}"] = v

            return await super().run(agent, *messages, **kwargs)
