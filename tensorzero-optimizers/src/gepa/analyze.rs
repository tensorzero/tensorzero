...
pub struct InferenceWithAnalysis {
    ...
    /// Optional inference input (only included if include_inference_input_for_mutation is true)
    ...
    let analysis_futures: Vec<_> = evaluation_infos
        ...
        .map(|(index, eval_info)| {
            ...
            async move {
                ...
                // Conditionally include inference input based on config flag
                let inference_input = if gepa_config.include_inference_input_for_mutation {
