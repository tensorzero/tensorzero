...
#[tokio::test(flavor = "multi_thread")]
async fn test_evaluate_multiple_variants() {
    let clickhouse = get_clickhouse().await;
    let http_client = TensorzeroHttpClient::new_testing().unwrap();
    let config = get_e2e_config().await;
    let gepa_config = create_gepa_config_chat();
    let dataset_name = "test_evaluate_multiple_variants".to_string();
    ...
    // Evaluate all variants (should have 2: openai and anthropic)
    assert!(
        initial_variants.len() >= 2,
        "Expected at least 2 initial variants"
    );
    ...
    for (variant_name, variant_config) in &initial_variants {
        ...
        assert_eq!(
            evaluation_results.evaluation_infos.len(),
            2,
            "Expected 2 evaluation results for variant '{variant_name}'"
        );
