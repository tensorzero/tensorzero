...
interface ContentBlockDisplayProps {
  block: StoredInputMessageContent;
  isEditing?: boolean;
  onChange?: (updatedContentBlock: StoredInputMessageContent) => void;
  action?: ReactNode;
  thoughtFooter?: (
...
export function ContentBlockDisplay({
  ...
  action,
  thoughtFooter,
}: ContentBlockDisplayProps): ReactNode {
  switch (block.type) {
    ...
    case "text": {
      if (typeof block.value === "string") {
        return (
          <TextMessage
            ...
            onChange={(updatedText) => {
              onChange?.({ ...block, value: updatedText });
            }}
            action={action}
          />
        );
      } else {
        // TODO: do we have to deal with the legacy format here...?
        // return (
        //   <TextMessage
        //     label="Text"
        //     content={block.value}
        ...
        return (
          <TextMessage
            label="Text"
            content={"TODO - LEGACY INPUT - NOT SUPPORTED HERE"}
            action={action}
          />
        );
      }
    ...
    case "tool_call": {
      return (
        <ToolCallMessage
          ...
          toolRawArguments={block.arguments}
          toolCallId={block.id}
          isEditing={isEditing}
          onChange={
            isEditing
