...
test("playground should run evaluation after inference", async ({ page }) => {
  ...
  // Select evaluation 'haiku' (configured for write_haiku function)
  await page.getByPlaceholder("Select evaluation").click();
  await page.getByRole("option", { name: "haiku" }).click();

  // Verify evaluation results appear (evaluator names shown in badges)
  await expect(page.getByText("exact_match")).toBeVisible({ timeout: 30_000 });
  await expect(page.getByText("topic_starts_with_f")).toBeVisible();
