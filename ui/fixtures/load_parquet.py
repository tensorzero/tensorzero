...
def format_value_for_copy(
    value, col_name: str, table_name: str, json_fields: set, jsonb_array_fields: set
):
    ...
    # finish_reason - map invalid values to 'other'
...
# NOTE: timestamp is excluded - it's a generated column derived from the UUIDv7 id
TABLE_CONFIGS = {
    "chat_inference": {
        "columns": [
            "id",
    ...
    try:
        ...
        for batch in parquet_file.iter_batches(
            ...
            for col in available_columns:
                df[col] = df[col].apply(
                    lambda v, c=col: format_value_for_copy(
                        v, c, table_name, json_fields, jsonb_array_fields
                    )
                )
            ...
            print(
                f" done. {total_inserted}/{total_rows} ({100*total_inserted//total_rows}%)",
                flush=True,
            )
