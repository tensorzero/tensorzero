...
impl TryFrom<Credential> for HyperbolicCredentials {
    ...
    fn try_from(credentials: Credential) -> Result<Self, Error> {
        match credentials {
            ...
            Credential::Missing => Ok(HyperbolicCredentials::Missing),
...
impl HyperbolicCredentials {
    fn get_api_key<'a>(
        ...
        match self {
            ...
            HyperbolicCredentials::Missing => Err(ErrorDetails::ApiKeyMissing {
                provider_name: PROVIDER_NAME.to_string(),
            })?,
        }
    }
...
impl InferenceProvider for HyperbolicProvider {
    ...
    fn is_missing_credentials(&self) -> bool {
        matches!(self.credentials, HyperbolicCredentials::Missing)
    }
}
...
mod tests {
    ...
    fn test_credential_to_hyperbolic_credentials() {
        ...
        let creds = HyperbolicCredentials::try_from(generic).unwrap();
        assert!(matches!(creds, HyperbolicCredentials::Missing));
