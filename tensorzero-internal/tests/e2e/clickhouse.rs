...
async fn test_clickhouse_migration_manager() {
    ...
    async fn twelfth(clickhouse: &ClickHouseConnectionInfo) {
        ...
        clickhouse_migration_manager::run_migration(&Migration0015 { clickhouse })
            .await
            .unwrap();
        ...
        assert!(!logs_contain("Applying migration: Migration0014"));
        assert!(!logs_contain("Migration succeeded: Migration0014"));
        assert!(logs_contain("Applying migration: Migration0015"));
        assert!(logs_contain("Migration succeeded: Migration0015"));
    }

    #[traced_test]
    async fn thirteenth(clickhouse: &ClickHouseConnectionInfo) {
        // Run the migration manager again (it should've already been run above)... there should be no changes
        let clean_start =
            clickhouse_migration_manager::run_migration(&Migration0000 { clickhouse })
                .await
