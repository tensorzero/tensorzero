...
mod erasure_tests {
    ...
    fn erased_task_tool_wrapper_exposes_name() {
        let wrapper = ErasedTaskToolWrapper::new(EchoTaskTool);
    ...
    fn erased_task_tool_wrapper_default_timeout() {
        let wrapper = ErasedTaskToolWrapper::new(DefaultTimeoutTaskTool);
        assert_eq!(wrapper.timeout(), Duration::from_secs(60));
    }
    ...
    fn erased_simple_tool_exposes_metadata() {
        let tool: &dyn ErasedTool = &EchoSimpleTool;
    ...
    fn erased_simple_tool_default_timeout() {
        let tool: &dyn ErasedTool = &DefaultTimeoutSimpleTool;
