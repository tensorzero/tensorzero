...
function main() {
  try {
    ...
    const missingExports = generatedFiles.filter(file => !exportedModules.includes(file));

    // Find extra exports (shouldn't happen with generated files, but good to check)
    const extraExports = exportedModules.filter(module => !generatedFiles.includes(module));
