...
/// Test that `row_limit` caps the total number of rows fetched (limit < PAGE_SIZE).
#[tokio::test]
async fn test_upload_dataset_parquet_with_row_limit() {
    // Dataset has 250 rows, but we only want 100.
    let datapoints: Vec<_> = (0..100)
        .map(|_| create_mock_chat_datapoint(Uuid::now_v7(), "test_dataset", "test_function"))
        .collect();

    let all_datapoints = datapoints.clone();
    let response = create_mock_get_datapoints_response(datapoints);

    let mut mock_client = MockTensorZeroClient::new();

    // With row_limit=100 (< PAGE_SIZE=1000), the function should request
    // min(PAGE_SIZE, remaining) = 100 rows and stop after getting them.
    mock_client
        .expect_list_datapoints()
        .times(1)
        .withf(|name, req| {
    ...
    assert_eq!(total_rows, 100, "Expected exactly 100 rows to be uploaded");
    ...
    let total: usize = batches.iter().map(|b| b.num_rows()).sum();
    assert_eq!(total, 100, "Expected 100 total rows in Parquet file");
...
/// Test that `row_limit` > PAGE_SIZE still paginates in chunks of PAGE_SIZE
/// and stops at the row_limit.
#[tokio::test]
async fn test_upload_dataset_parquet_with_row_limit_multi_page() {
    // row_limit=1500 means: page1 of 1000 (PAGE_SIZE), page2 of 500 (remaining)
    let datapoints_page1: Vec<_> = (0..1000)
        .map(|_| create_mock_chat_datapoint(Uuid::now_v7(), "test_dataset", "test_function"))
        .collect();
    let datapoints_page2: Vec<_> = (0..500)
    ...
    // First page: requests PAGE_SIZE=1000 (min(1000, 1500))
    mock_client
        .expect_list_datapoints()
        .times(1)
        .in_sequence(&mut seq)
    ...
    // Second page: requests 500 (min(1000, 1500-1000))
    mock_client
        .expect_list_datapoints()
        .times(1)
        .in_sequence(&mut seq)
        .withf(|name, req| {
            name == "test_dataset" && req.offset == Some(1000) && req.limit == Some(500)
        })
        .return_once(move |_, _| Ok(response2));
