...
TAG=tensorzero/openai-node-client-tests:ci-sha-$SHORT_HASH
LATEST_TAG=tensorzero/openai-node-client-tests:latest
...
docker build --load --build-arg BUILDKIT_CONTEXT_KEEP_GIT_DIR=1 \
  ...
  -f clients/openai-node/tests/Dockerfile . -t $TAG
