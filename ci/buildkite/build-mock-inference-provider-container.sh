...
# docker pull tensorzero/mock-inference-provider:latest || true

# Build the container with cache
docker build --load --build-arg BUILDKIT_CONTEXT_KEEP_GIT_DIR=1 \
  --cache-from tensorzero/mock-inference-provider:latest \
  -f tensorzero-core/tests/mock-inference-provider/Dockerfile . -t $TAG

# Tag with latest and push both tags
docker tag $TAG tensorzero/mock-inference-provider:latest
echo "Pushing $TAG"
docker push $TAG
echo "Pushing tensorzero/mock-inference-provider:latest"
docker push tensorzero/mock-inference-provider:latest
