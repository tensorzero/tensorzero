...
TAG=tensorzero/provider-proxy-cache-dev:ci-sha-$SHORT_HASH
...
docker pull tensorzero/provider-proxy-cache-dev:latest || true

# Build the container with cache
docker build --load --build-arg BUILDKIT_CONTEXT_KEEP_GIT_DIR=1 \
  --cache-from tensorzero/provider-proxy-cache-dev:latest \
  -f ci/provider-proxy-cache/Dockerfile . -t $TAG

# Tag with latest and push both tags
docker tag $TAG tensorzero/provider-proxy-cache-dev:latest
echo "Pushing $TAG"
docker push $TAG
echo "Pushing tensorzero/provider-proxy-cache-dev:latest"
docker push tensorzero/provider-proxy-cache-dev:latest
