...
async fn test_clickhouse_migration_manager() {
    ...
    let response = clickhouse
        .run_query_synchronous_no_params(
            "SELECT count FROM CumulativeUsageV2 FINAL WHERE type='input_tokens'".to_string(),
        )
        .await
        .unwrap();
    ...
    let response = clickhouse
        .run_query_synchronous_no_params(
            "SELECT count FROM CumulativeUsageV2 FINAL WHERE type='output_tokens'".to_string(),
        )
        .await
        .unwrap();
