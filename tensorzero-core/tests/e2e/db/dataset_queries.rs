...
async fn test_insert_chat_datapoint_snapshot_hash_postgres() {
    ...
    let row =
        sqlx::query("SELECT snapshot_hash FROM tensorzero.chat_inference_datapoints WHERE id = $1")
            .bind(datapoint_id)
            .fetch_one(pool)
            .await
            .unwrap();
...
async fn test_insert_json_datapoint_snapshot_hash_postgres() {
    ...
    let pool = postgres.get_pool().unwrap();
    let row =
        sqlx::query("SELECT snapshot_hash FROM tensorzero.json_inference_datapoints WHERE id = $1")
            .bind(datapoint_id)
            .fetch_one(pool)
            .await
            .unwrap();
