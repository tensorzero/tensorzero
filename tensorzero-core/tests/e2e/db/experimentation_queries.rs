...
async fn test_cas_performance_baseline(pool: PgPool) {
    ...
    let result = conn
        ...
        .await
        .unwrap();
    let _single_op_duration = start.elapsed();

    assert_eq!(result, variant_name);
    // Single CAS operation timing recorded

    // Measure repeated operations (should be faster due to existing value)
    let repeated_ops = 100;
    let start = Instant::now();

    for i in 0..repeated_ops {
        let attempt_variant = format!("perf_variant_{i}");
        let result = conn
            .check_and_set_variant_by_episode(episode_id, &function_name, &attempt_variant)
            .await
            .unwrap();
        assert_eq!(
            result, variant_name,
            "Should always return original variant"
        );
    }

    let repeated_ops_duration = start.elapsed();
    let avg_repeated_duration = repeated_ops_duration / repeated_ops;

    // Average repeated CAS operation timing recorded
    // Total repeated operations timing recorded

    // Performance assertion: repeated operations should be reasonably fast
    // This is a baseline - adjust threshold based on actual performance requirements
    assert!(
        avg_repeated_duration.as_millis() < 100,
        "Average repeated operation took too long: {avg_repeated_duration:?}"
    );
}
