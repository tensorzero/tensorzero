/// Tests for the GET /v1/datasets endpoint (list_datasets).
...
async fn test_list_datasets_no_params() {
    ...
    for _ in 0..10 {
        ...
        let resp = http_client
            .get(get_gateway_endpoint("/v1/datasets"))
            .send()
            .await
            .unwrap();
    ...
    for _ in 0..10 {
        ...
        let resp = http_client
            .get(get_gateway_endpoint(&format!(
                "/v1/datasets?function_name={function_name_1}",
            )))
            .send()
            .await
            .unwrap();
...
async fn test_list_datasets_with_pagination() {
    ...
    // Test limit
    let resp = http_client
        .get(get_gateway_endpoint("/v1/datasets?limit=1"))
        .send()
        .await
        .unwrap();
    ...
    let resp_page_1 = http_client
        .get(get_gateway_endpoint("/v1/datasets?limit=1&offset=0"))
        .send()
        .await
        .unwrap();

    let resp_page_2 = http_client
        .get(get_gateway_endpoint("/v1/datasets?limit=1&offset=1"))
        .send()
        .await
        .unwrap();
...
async fn test_list_datasets_empty_result() {
    ...
    let resp = http_client
        .get(get_gateway_endpoint(&format!(
            "/v1/datasets?function_name={nonexistent_function}",
        )))
        .send()
        .await
        .unwrap();
