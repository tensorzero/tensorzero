...
async fn test_query_episode_table() {
    ...
    let episodes: Vec<EpisodeByIdRow> = resp.json().await.unwrap();
...
async fn test_query_episode_table_with_pagination() {
    ...
    let first_page: Vec<EpisodeByIdRow> = first_page_resp.json().await.unwrap();

    if first_page.len() == 5 && bounds.count > 5 {
        // If we have more episodes, test pagination with 'before'
        let last_episode_id = first_page.last().unwrap().episode_id;
        ...
        let second_page: Vec<EpisodeByIdRow> = second_page_resp.json().await.unwrap();

        // Ensure no overlap between pages (all IDs in second page should be different)
        for episode in &second_page {
            assert!(
                !first_page
                    .iter()
...
async fn test_query_episode_table_limit_zero() {
    ...
    let episodes: Vec<EpisodeByIdRow> = resp.json().await.unwrap();
    assert!(episodes.is_empty(), "Expected empty result for limit=0");
}
