...
async fn test_get_latest_feedback_by_metric_with_data() {
    ...
    let url = get_gateway_endpoint(&format!(
        "/internal/latest-feedback-by-metric/{inference_id}"
    ));
    let resp = http_client.get(url).send().await.unwrap();

    assert!(
        resp.status().is_success(),
        "get_latest_feedback_by_metric request failed: status={:?}",
        resp.status()
    );
...
#[tokio::test(flavor = "multi_thread")]
async fn test_get_latest_feedback_by_metric_multiple_feedback_same_metric() {
    let http_client = Client::new();
...
#[tokio::test(flavor = "multi_thread")]
async fn test_get_latest_feedback_by_metric_nonexistent_target() {
    let http_client = Client::new();

    // Use a UUID that likely doesn't exist
    let nonexistent_id = Uuid::now_v7();
    let url = get_gateway_endpoint(&format!(
        "/internal/latest-feedback-by-metric/{nonexistent_id}"
    ));
...
#[tokio::test(flavor = "multi_thread")]
async fn test_get_latest_feedback_by_metric_invalid_uuid() {
    let http_client = Client::new();

    // Use an invalid UUID
    let url = get_gateway_endpoint("/internal/latest-feedback-by-metric/not-a-valid-uuid");
