...
async fn insert_chat_inference(
    ...
    sqlx::query(
        r"INSERT INTO chat_inference (id, function_name, variant_name, episode_id, input, output, tags)
...
async fn insert_json_inference(
    ...
    sqlx::query(
        r"INSERT INTO json_inference (id, function_name, variant_name, episode_id, input, output, tags)
           VALUES ($1, $2, $3, $4, $5, $6, $7)",
    )
    .bind(id)
    .bind(function_name)
