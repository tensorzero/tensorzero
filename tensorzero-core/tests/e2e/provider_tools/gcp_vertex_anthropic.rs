//! GCP Vertex Anthropic provider tools E2E tests (e.g. web_search, bash)
//!
...
/// Test GCP Vertex Anthropic dynamic provider tools with web_search (non-streaming)
/// This test passes provider_tools via the request instead of the model config
#[tokio::test(flavor = "multi_thread")]
pub async fn test_gcp_vertex_anthropic_dynamic_provider_tools_web_search_nonstreaming() {
...
"#;
    ...
    let response = result.unwrap();
    println!("GCP Vertex Anthropic dynamic web_search response: {response:?}");
    ...
    println!("GCP Vertex Anthropic dynamic round-trip response: {response2:?}");
...
/// Test GCP Vertex Anthropic dynamic provider tools with web_search (streaming)
/// This test passes provider_tools via the request instead of the model config
#[tokio::test(flavor = "multi_thread")]
pub async fn test_gcp_vertex_anthropic_dynamic_provider_tools_web_search_streaming() {
    // Config WITHOUT provider_tools - they will be passed dynamically
    let config = r#"
gateway.debug = true
...
"#;
    ...
    let response = result.unwrap();
    println!("GCP Vertex Anthropic dynamic web_search streaming response: {response:?}");
