...
macro_rules! generate_provider_tests {
    ($func:ident) => {
        ...
        #[tokio::test(flavor = "multi_thread")]
        async fn test_multi_turn_parallel_tool_use_nonstreaming() {
            let providers = $func().await.parallel_tool_use_inference;
            for provider in providers {
                $crate::providers::commonv2::multi_turn_parallel_tool_use::test_multi_turn_parallel_tool_use_with_provider(provider).await;
            }
        }

        #[tokio::test(flavor = "multi_thread")]
        async fn test_multi_turn_parallel_tool_use_streaming() {
            let providers = $func().await.parallel_tool_use_inference;
            for provider in providers {
                $crate::providers::commonv2::multi_turn_parallel_tool_use::test_multi_turn_parallel_tool_use_streaming_with_provider(provider).await;
            }
        }
