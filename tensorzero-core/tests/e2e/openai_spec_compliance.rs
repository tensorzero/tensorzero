...
fn get_component_schema(component_name: &str) -> Value {
    ...
    json!({
        "$defs": components,
        "$ref": format!("#/$defs/{}", component_name)
    })
}
...
fn assert_valid_schema(schema: &JSONSchema, instance: &Value, context: &str) {
    if let Err(errors) = schema.validate(instance) {
        ...
        panic!(
            ...
            context,
            error_msgs.iter().map(|e| format!("  - {}", e)).collect::<Vec<_>>().join("\n"),
...
/// Get the Error response schema (for 4xx/5xx responses)
...
/// Get the ChatCompletion response schema
...
/// Get the Embeddings response schema
