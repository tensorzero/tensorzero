    ...
    async fn get_config_snapshot(
        ...
        .map_err(|e| {
            Error::new(ErrorDetails::PostgresQuery {
                message: format!("Failed to get config snapshot: {e}"),
            })
        })?;
        ...
        let config: String = row.try_get("config").map_err(|e| {
            Error::new(ErrorDetails::PostgresQuery {
                message: format!("Failed to get config column: {e}"),
            })
        })?;

        let extra_templates_json: serde_json::Value =
            row.try_get("extra_templates").map_err(|e| {
                Error::new(ErrorDetails::PostgresQuery {
                    message: format!("Failed to get extra_templates column: {e}"),
                })
            })?;

        let tags_json: serde_json::Value = row.try_get("tags").map_err(|e| {
            Error::new(ErrorDetails::PostgresQuery {
                message: format!("Failed to get tags column: {e}"),
            })
        })?;

        let extra_templates: HashMap<String, String> = serde_json::from_value(extra_templates_json)
            .map_err(|e| {
                Error::new(ErrorDetails::PostgresQuery {
                    message: format!("Failed to deserialize extra_templates: {e}"),
                })
            })?;

        let tags: HashMap<String, String> = serde_json::from_value(tags_json).map_err(|e| {
            Error::new(ErrorDetails::PostgresQuery {
    ...
    async fn write_config_snapshot(&self, snapshot: &ConfigSnapshot) -> Result<(), Error> {
        ...
        let extra_templates_json =
            serde_json::to_value(&snapshot.extra_templates).map_err(|e| {
                Error::new(ErrorDetails::Serialization {
                    message: format!("Failed to serialize extra_templates: {e}"),
                })
            })?;
        ...
        .map_err(|e| {
            Error::new(ErrorDetails::PostgresQuery {
                message: format!("Failed to write config snapshot: {e}"),
            })
        })?;
