...
pub(super) fn build_insert_chat_inferences_query(
    ...
    let mut query_builder: QueryBuilder<sqlx::Postgres> = QueryBuilder::new(
        ...
        INSERT INTO tensorzero.chat_inferences (
            ...
            tool_choice, parallel_tool_calls, snapshot_hash, created_at
        ) ",
    );
...
pub(super) fn build_insert_chat_inference_data_query(
    ...
    let default_tool_params = ToolCallConfigDatabaseInsert::default();

    let mut query_builder: QueryBuilder<sqlx::Postgres> = QueryBuilder::new(
        r"
        INSERT INTO tensorzero.chat_inference_data (
            id, input, output, tool_params, inference_params,
    ...
    query_builder.push_values(rows.iter().zip(&timestamps), |mut b, (row, created_at)| {
        ...
        b.push_bind(row.id)
            ...
            .push_bind(Json(
                row.tool_params.as_ref().unwrap_or(&default_tool_params),
            ))
...
fn push_tsvector_search_filter(
    ...
    qb.push(format!(
        " AND ({input_tsvector_col} @@ websearch_to_tsquery('simple',"
    ));
    qb.push_bind(search_query.to_string());
    qb.push(format!(
        ") OR {output_tsvector_col} @@ websearch_to_tsquery('simple',"
    ));
    qb.push_bind(search_query.to_string());
    qb.push("))");
}
...
enum OrderByTermResolved {
    ...
    Column(String),
    /// A ts_rank expression for search relevance ordering.
        ...
        for (i, (term, direction)) in self.terms.iter().enumerate() {
            ...
            match term {
                ...
                OrderByTermResolved::SearchRelevance(query) => {
                    qb.push(
                        "ts_rank(io.input_tsvector || io.output_tsvector, websearch_to_tsquery('simple',",
                    );
...
fn build_inferences_union_query(
    ...
    if let Some(sq) = search_query_for_rank {
        ...
        query_builder.push_bind(sq.to_string());
        query_builder.push(")) AS search_rank");
    }
...
mod tests {
    ...
    fn test_build_chat_inferences_query_with_search() {
        ...
        assert_query_equals(
            ...
            LEFT JOIN tensorzero.chat_inference_data io ON io.id = i.id AND io.created_at = i.created_at
            WHERE 1=1 AND i.function_name = $1
            AND (io.input_tsvector @@ websearch_to_tsquery('simple',$2) OR io.output_tsvector @@ websearch_to_tsquery('simple',$3))
            ORDER BY i.id DESC
            LIMIT $4 OFFSET $5
            ",
        );
