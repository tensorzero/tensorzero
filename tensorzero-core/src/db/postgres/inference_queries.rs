...
impl InferenceQueries for PostgresConnectionInfo {
    ...
    async fn insert_chat_inferences(
        ...
        // Pre-compute timestamps so we can return errors properly
        let created_at_timestamps: Vec<DateTime<Utc>> = rows
            .iter()
        ...
        query_builder.push_values(
            rows.iter().zip(&created_at_timestamps),
        ...
        query_builder.push_values(
            ...
            |mut b, (row, created_at)| {
                ...
                let output_schema_json =
                    serde_json::to_value(&row.output_schema).unwrap_or_default();
                let inference_params_json =
                    serde_json::to_value(&row.inference_params).unwrap_or_default();
                let tags_json = serde_json::to_value(&row.tags).unwrap_or_default();
                let extra_body_json = serde_json::to_value(&row.extra_body).unwrap_or_default();
...
#[expect(
    dead_code,
    reason = "tool_params field is selected by sqlx but not used in conversion"
)]
    ...
    fn from_row(row: &'r sqlx::postgres::PgRow) -> Result<Self, sqlx::Error> {
        ...
        match inference_type.as_str() {
            "chat" => {
                ...
                let tool_choice: Option<Value> = row.try_get("tool_choice")?;
...
fn serialize_tool_params(
    ...
    match tool_params {
        Some(tp) => {
            ...
            let tool_choice = serde_json::to_value(&tp.tool_choice).map_err(|e| {
                Error::new(ErrorDetails::Serialization {
