...
impl DICLQueries for PostgresConnectionInfo {
    async fn insert_dicl_example(&self, example: &StoredDICLExample) -> Result<(), Error> {
        ...
        // Convert embedding to string format for pgvector: "[0.1,0.2,0.3]"
        let embedding_str = format_embedding_for_postgres(&example.embedding);
        ...
        .bind(example.id)
        .bind(&example.function_name)
        .bind(&example.variant_name)
        .bind(&example.namespace)
        .bind(&example.input)
        .bind(&example.output)
        .bind(&embedding_str)
