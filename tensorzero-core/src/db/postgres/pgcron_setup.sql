...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        ...
        PERFORM cron.schedule(
            ...
            '30 0 * * *',
            $$
            SELECT tensorzero.drop_old_monthly_partitions('chat_inferences', 'inference_retention_days');
            SELECT tensorzero.drop_old_monthly_partitions('json_inferences', 'inference_retention_days');
            SELECT tensorzero.drop_old_monthly_partitions('model_inferences', 'inference_retention_days');
            SELECT tensorzero.drop_old_partitions('chat_inference_data', 'inference_retention_days');
            SELECT tensorzero.drop_old_partitions('json_inference_data', 'inference_retention_days');
            SELECT tensorzero.drop_old_partitions('model_inference_data', 'inference_retention_days');
            $$
        );
