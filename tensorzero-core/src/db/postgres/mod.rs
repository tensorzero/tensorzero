...
impl PostgresConnectionInfo {
    ...
    /// Writes retention configuration to the `tensorzero_retention_config` table.
    ...
    pub async fn write_retention_config(
        ...
        match inference_retention_days {
            Some(days) => {
                sqlx::query(
                    r"
                    INSERT INTO tensorzero_retention_config (key, value, updated_at)
            ...
            None => {
                sqlx::query(
                    "DELETE FROM tensorzero_retention_config WHERE key = 'inference_retention_days'",
                )
                .execute(pool)
                .await
                .map_err(|e| {
