...
/// PostgreSQL uses lowercase snake_case table names.
fn pg_table_name(function_type: FunctionConfigType) -> &'static str {
    match function_type {
        FunctionConfigType::Chat => "chat_inference",
        FunctionConfigType::Json => "json_inference",
    }
}
...
fn time_window_to_pg_interval(time_window: &TimeWindow) -> &'static str {
    match time_window {
...
impl InferenceCountQueries for PostgresConnectionInfo {
    ...
    async fn count_inferences_for_episode(&self, episode_id: uuid::Uuid) -> Result<u64, Error> {
        ...
        let count: i64 = sqlx::query_scalar(
            r"SELECT COALESCE(SUM(count), 0)::bigint AS count FROM (
                SELECT COUNT(*) AS count FROM chat_inference WHERE episode_id = $1
                UNION ALL
                SELECT COUNT(*) AS count FROM json_inference WHERE episode_id = $1
            ) t",
        )
        .bind(episode_id)
        .fetch_one(pool)
        ...
        let rows = match params.time_window {
            TimeWindow::Cumulative => {
                ...
                sqlx::query(
                    ...
                    FROM (
                        SELECT variant_name FROM chat_inference WHERE function_name = $1
                        UNION ALL
                        SELECT variant_name FROM json_inference WHERE function_name = $1
                    ) t
                    GROUP BY variant_name
            ...
            _ => {
                ...
                let interval_str = time_window_to_pg_interval(&params.time_window);
    ...
    async fn list_functions_with_inference_count(
        ...
        let rows = sqlx::query(
            ...
            FROM (
                SELECT function_name, timestamp FROM chat_inference
                UNION ALL
                SELECT function_name, timestamp FROM json_inference
            ) t
