...
fn table_name(function_type: FunctionConfigType) -> &'static str {
    match function_type {
        FunctionConfigType::Chat => "chat_inferences",
        FunctionConfigType::Json => "json_inferences",
    }
}
...
async fn count_inferences(
    ...
    let table = table_name(function_type);
    ...
    let rows = if time_window == TimeWindow::Cumulative {
        ...
        let mut qb = QueryBuilder::new(
            r"SELECT
                ...
                COUNT(*)::INT AS count
            FROM (
                SELECT variant_name FROM chat_inferences WHERE function_name = ",
        );
        qb.push_bind(function_name);
        qb.push(" UNION ALL SELECT variant_name FROM json_inferences WHERE function_name = ");
        qb.push_bind(function_name);
        ...
        let mut qb = QueryBuilder::new(
            "WITH combined AS (
                SELECT variant_name, created_at FROM chat_inferences WHERE function_name = ",
        );
        qb.push_bind(function_name);
        qb.push(" UNION ALL SELECT variant_name, created_at FROM json_inferences WHERE function_name = ");
        qb.push_bind(function_name);
        qb.push(
            "),
            max_time AS (
...
impl InferenceCountQueries for PostgresConnectionInfo {
    ...
    async fn count_inferences_with_feedback(
        ...
        // TODO(#XXX): Implement when feedback tables are added in step-2
    ...
    async fn count_inferences_for_episode(&self, episode_id: uuid::Uuid) -> Result<u64, Error> {
        ...
        let mut qb = QueryBuilder::new(
            r"SELECT COUNT(*) FROM (
                SELECT id FROM chat_inferences WHERE episode_id = ",
        );
        qb.push_bind(episode_id);
        qb.push(" UNION ALL SELECT id FROM json_inferences WHERE episode_id = ");
        qb.push_bind(episode_id);
        qb.push(") AS combined");
    ...
    async fn list_functions_with_inference_count(
        ...
        let rows = sqlx::query(
            ...
            FROM (
                SELECT function_name, created_at FROM chat_inferences
                UNION ALL
                SELECT function_name, created_at FROM json_inferences
            ) AS combined
