...
impl InferenceProvider for MistralProvider {
    async fn infer<'a>(
        ...
        let request_body = serde_json::to_value(
            MistralRequest::new(&self.model_name, request, model_name, provider_name).await?,
        )
        .map_err(|e| {
...
impl<'a> From<&'a FunctionToolConfig> for MistralFunctionTool<'a> {
...
pub(super) fn prepare_mistral_tools<'a>(
    ...
    match &request.tool_config {
        ...
        Some(tool_config) => {
            ...
            let mut tools: Vec<MistralTool<'a>> = tool_config
                .strict_tools_available()?
                .map(|tool| MistralTool::Function(tool.into()))
                .collect();

            // Add provider tools (e.g., web_search, code_interpreter)
            tools.extend(
                provider_tools
                    .iter()
...
impl<'a> MistralRequest<'a> {
    pub async fn new(
        ...
        let (tools, tool_choice, _) =
            prepare_mistral_tools(request, tensorzero_model_name, tensorzero_provider_name)?;
...
mod tests {
    ...
    async fn test_mistral_request_new() {
        ...
        let MistralTool::Function(tool) = &tools[0] else {
    ...
    fn test_prepare_mistral_tools_with_allowed_tools() {
        ...
        let (tools, tool_choice, parallel_tool_calls) =
            prepare_mistral_tools(&request, "test-model", "test-provider").unwrap();
    ...
    #[test]
    fn test_prepare_mistral_tools_provider_only() {
        use crate::providers::test_helpers::provider_only_tool_config;

        let tool_config = provider_only_tool_config("test-model", "mistral");
        let request = ModelInferenceRequest {
            inference_id: Uuid::now_v7(),
            messages: vec![RequestMessage {
                role: Role::User,
        ...
        let result = prepare_mistral_tools(&request, "test-model", "mistral");
        assert!(
            result.is_ok(),
            "prepare_mistral_tools should succeed with provider-only tools"
        );
        ...
        match &tools[0] {
            ...
            MistralTool::Function(_) => panic!("Expected a ProviderTool"),
        }
