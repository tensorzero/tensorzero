...
#[derive(Debug, Serialize)]
#[serde(untagged)]
enum GeminiToolEntry<'a> {
    Function(GeminiTool<'a>),
...
struct GeminiRequest<'a> {
    ...
    tools: Option<Vec<GeminiToolEntry<'a>>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    match &request.tool_config {
        Some(tool_config) => {
            ...
            tools.extend(
                provider_tools
                    .iter()
                    .map(|t| GeminiToolEntry::ProviderTool(&t.tool)),
            );
...
mod tests {
    ...
    async fn test_google_ai_studio_gemini_request_try_from() {
        ...
        let result = GeminiRequest::new(&inference_request, "test-model", "test-provider").await;
    ...
    fn test_prepare_tools() {
        ...
        let (tools, tool_choice) =
            prepare_tools(&request_with_tools, "test-model", "test-provider").unwrap();
        ...
        match &tools[0] {
            GeminiToolEntry::Function(GeminiTool {
            ...
            GeminiToolEntry::ProviderTool(_) => panic!("Expected a function tool"),
        }
        let request_with_tools = ModelInferenceRequest {
            inference_id: Uuid::now_v7(),
            messages: vec![RequestMessage {
    ...
    fn test_prepare_tools_provider_only() {
        ...
        let tool_config = provider_only_tool_config("test-model", "google_ai_studio_gemini");
        ...
        let result = prepare_tools(&request, "test-model", "google_ai_studio_gemini");
        assert!(
            result.is_ok(),
        ...
        let has_provider_tool = tools
            .iter()
            .any(|t| matches!(t, GeminiToolEntry::ProviderTool(_)));
        assert!(
            has_provider_tool,
