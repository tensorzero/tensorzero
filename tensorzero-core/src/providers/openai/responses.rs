    ...
    fn test_convert_unknown_block_to_content_block_output() {
        ...
        match &provider_response.output[0] {
            ContentBlockOutput::Unknown(unknown) => {
                assert_eq!(
                    unknown.data.get("type").and_then(|v| v.as_str()),
                    Some("web_search_call")
                );
                assert_eq!(unknown.model_name.as_deref(), Some("test-model"));
                assert_eq!(unknown.provider_name.as_deref(), Some("test-provider"));
            }
    ...
    fn test_mixed_output_with_known_and_unknown_blocks() {
        ...
        match &provider_response.output[3] {
            ContentBlockOutput::Unknown(unknown) => {
                ...
                assert_eq!(
                    unknown.data.get("custom_field").and_then(|v| v.as_str()),
                    Some("custom_value")
                );
                assert_eq!(unknown.model_name.as_deref(), Some("gpt-5-nano"));
                assert_eq!(unknown.provider_name.as_deref(), Some("openai"));
            }
