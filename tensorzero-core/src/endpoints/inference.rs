...
async fn write_inference<T: InferenceQueries + Send + Sync>(
    ...
    match result {
        InferenceResult::Chat(result) => {
            ...
            let chat_inference = ChatInferenceDatabaseInsert::new(result, stored_input, metadata);
        ...
        InferenceResult::Json(result) => {
            let stored_input = input.clone().into_stored_input();
            let json_inference = JsonInferenceDatabaseInsert::new(result, stored_input, metadata);
            let _ = inference_database
