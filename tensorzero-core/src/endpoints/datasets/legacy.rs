...
pub struct CloneDatapointsResponse {
    pub ids: Vec<Option<Uuid>>, // None for missing source datapoints
}

/// The handler for the POST `/v1/datasets/{dataset_name}/datapoints/clone` endpoint.
...
pub async fn clone_datapoints_handler(
    ...
    Ok(Json(CloneDatapointsResponse { ids: new_ids }))
}
...
pub async fn clone_datapoints(
    ...
    // Generate all mappings optimistically (no pre-check needed)
    ...
    // CTE includes join column (new_id), EXCEPT removes it, REPLACE substitutes values
    ...
    // Round trip 2: Verify which new_ids were created
        ...
        .map(|(source_id, new_id)| {
            if created_ids.contains(new_id) {
                ...
                tracing::warn!(
                    ...
                    "Source datapoint not found during clone"
                );
                None
            }
        })
