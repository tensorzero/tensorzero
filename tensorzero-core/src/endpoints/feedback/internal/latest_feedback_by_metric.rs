...
#[instrument(
    name = "get_latest_feedback_by_metric_handler",
    skip_all,
    fields(
        target_id = %target_id,
    )
)]
pub async fn get_latest_feedback_by_metric_handler(
    State(app_state): AppState,
    Path(target_id): Path<Uuid>,
) -> Result<Json<LatestFeedbackIdByMetricResponse>, Error> {
    let response =
        get_latest_feedback_by_metric(&app_state.clickhouse_connection_info, target_id).await?;
    Ok(Json(response))
}

/// Core business logic for getting the latest feedback ID for each metric
pub async fn get_latest_feedback_by_metric(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_latest_feedback_by_metric_calls_clickhouse() {
        ...
        let result = get_latest_feedback_by_metric(&mock_clickhouse, target_id)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_get_latest_feedback_by_metric_empty_result() {
        let mut mock_clickhouse = MockFeedbackQueries::new();
