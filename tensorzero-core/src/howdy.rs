...
async fn get_token_totals(
    ...
    let Ok(response) = clickhouse
        .run_query_synchronous_no_params(
            r"
            SELECT
                (SELECT SUM(count) FROM CumulativeUsage FINAL WHERE type = 'input_tokens') as input_tokens,
                (SELECT SUM(count) FROM CumulativeUsage FINAL WHERE type = 'output_tokens') as output_tokens
            Format JSONEachRow
            "
            .to_string(),
        )
