...
//! * `tensorzero.overhead.latency_ms` - Sets the total latency of a span with overhead tracking enabled.
//!   This must be applied to the same span as the `tensorzero.overhead.kind` attribute
...
//!
//!
//! In this example, we set the `tensorzero.overhead.latency_ms` value after [`send_response`] finished
//! (after we've finished sending our response body to the client.)
//! The overhead value is then computed as follows:
//! * The initial time interval starts at the instant the `request` span was created, and spans `tensorzero.overhead.latency_ms` milliseconds.
//!   Note that this does *not* include the time taken by `update_rate_limiting` - even though the `request` span was still active,
//!   the explicit `tensorzero.overhead.latency_ms` duration value was set to only include the time taken until `send_response` finished,
...
//! * The initial duration is controlled by setting the `tensorzero.overhead.latency_ms` span attribute.
...
//!   We currently set `tensorzero.overhead.latency_ms` attribute to the response body
...
pub const TENSORZERO_LATENCY_ATTRIBUTE_NAME: &str = "tensorzero.overhead.latency_ms";
