...
/// Serializes a `Vec<T>` as a JSON array of strings, where each element is first serialized to a JSON string.
/// This creates a "doubly-serialized" array useful for ClickHouse compatibility.
///
/// # Example
/// ```ignore
/// #[derive(Serialize)]
...
/// #[derive(Serialize)]
/// struct Container {
///     #[serde(serialize_with = "serialize_vec_as_json_strings")]
///     items: Vec<MyStruct>,
/// }
///
/// let container = Container {
///     items: vec![
///         MyStruct { foo: 1, bar: "a".to_string() },
///         MyStruct { foo: 2, bar: "b".to_string() },
///     ],
/// };
/// // Serializes to: {"items": ["{\"foo\":1,\"bar\":\"a\"}", "{\"foo\":2,\"bar\":\"b\"}"]}
/// ```
pub fn serialize_vec_as_json_strings<S, T>(value: &Vec<T>, serializer: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
    T: Serialize,
{
    ...
    for item in value {
        let json_str = serde_json::to_string(item).map_err(serde::ser::Error::custom)?;
        seq.serialize_element(&json_str)?;
    }
    seq.end()
}
...
mod tests {
    ...
    // Tests for serialize_vec_as_json_strings
    #[derive(Debug, Serialize, PartialEq)]
    struct TestVecAsJsonStrings {
        #[serde(serialize_with = "serialize_vec_as_json_strings")]
        items: Vec<TestStruct>,
    }

    #[test]
    fn test_serialize_vec_as_json_strings_empty() {
        let obj = TestVecAsJsonStrings { items: vec![] };
        let json = serde_json::to_string(&obj).unwrap();
        assert_eq!(json, r#"{"items":[]}"#);
    }

    #[test]
    fn test_serialize_vec_as_json_strings_single_element() {
        let obj = TestVecAsJsonStrings {
            items: vec![TestStruct {
                foo: 42,
                bar: "test".to_string(),
            }],
        ...
        let json = serde_json::to_string(&obj).unwrap();
        assert_eq!(json, r#"{"items":["{\"foo\":42,\"bar\":\"test\"}"]}"#);
    }

    #[test]
    fn test_serialize_vec_as_json_strings_multiple_elements() {
        let obj = TestVecAsJsonStrings {
            items: vec![
                TestStruct {
                    foo: 1,
        ...
        assert_eq!(
            json,
            r#"{"items":["{\"foo\":1,\"bar\":\"first\"}","{\"foo\":2,\"bar\":\"second\"}","{\"foo\":3,\"bar\":\"third\"}"]}"#
        );
    }

    #[test]
    fn test_serialize_vec_as_json_strings_with_special_chars() {
        let obj = TestVecAsJsonStrings {
            items: vec![TestStruct {
                foo: 1,
                bar: "with \"quotes\" and \\ backslash".to_string(),
            }],
        };
        let json = serde_json::to_string(&obj).unwrap();
        // The bar field should be doubly-escaped
        assert_eq!(
            json,
            r#"{"items":["{\"foo\":1,\"bar\":\"with \\\"quotes\\\" and \\\\ backslash\"}"]}"#
        );
    }

    #[test]
    fn test_serialize_vec_as_json_strings_roundtrip() {
        // Test that we can serialize and then deserialize each element
        let original = vec![
            TestStruct {
                foo: 1,
                bar: "a".to_string(),
        ...
        let parsed: serde_json::Value = serde_json::from_str(&json).unwrap();
        let items_array = parsed["items"].as_array().unwrap();

        // Each item should be a string
        assert_eq!(items_array.len(), 2);
        assert!(items_array[0].is_string());
        assert!(items_array[1].is_string());

        // Deserialize each string back to TestStruct
        let item0: TestStruct = serde_json::from_str(items_array[0].as_str().unwrap()).unwrap();
        let item1: TestStruct = serde_json::from_str(items_array[1].as_str().unwrap()).unwrap();

        assert_eq!(item0, original[0]);
        assert_eq!(item1, original[1]);
    }

    #[derive(Debug, Serialize, PartialEq)]
    struct TestVecAsJsonStringsPrimitives {
        #[serde(serialize_with = "serialize_vec_as_json_strings")]
        numbers: Vec<u32>,
    }

    #[test]
    fn test_serialize_vec_as_json_strings_primitives() {
        let obj = TestVecAsJsonStringsPrimitives {
            numbers: vec![1, 2, 3, 4, 5],
        };
        let json = serde_json::to_string(&obj).unwrap();
        // Each number should be serialized as a JSON string containing that number
        assert_eq!(json, r#"{"numbers":["1","2","3","4","5"]}"#);
    }
    ...
    #[derive(Debug, Serialize, PartialEq)]
    struct TestVecAsJsonStringsNested {
        #[serde(serialize_with = "serialize_vec_as_json_strings")]
        nested: Vec<NestedStruct>,
    }

    #[test]
    fn test_serialize_vec_as_json_strings_nested_structs() {
        let obj = TestVecAsJsonStringsNested {
            nested: vec![NestedStruct {
                inner: TestStruct {
                    foo: 1,
                    bar: "test".to_string(),
        ...
        assert_eq!(
            json,
            r#"{"nested":["{\"inner\":{\"foo\":1,\"bar\":\"test\"},\"count\":10}"]}"#
        );
    }
}
