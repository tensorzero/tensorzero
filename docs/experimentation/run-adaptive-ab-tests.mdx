...
You can use the OpenAI SDK pointed at the TensorZero Gateway.

```python
from openai import OpenAI

client = OpenAI(base_url="http://localhost:3000/openai/v1", api_key="unused")
...
The inference ID is available in the `id` field of the OpenAI-compatible response.
...
The adaptive experiment uses a track-and-stop algorithm for best-arm identification.
The algorithm exposes multiple customizable parameters.
...
## Run different experiments per namespace (e.g. customer)

You can configure different experiments per namespace (e.g. per customer).
See [Scope experiments with namespaces](/experimentation/scope-experiments-with-namespaces) for more details.
