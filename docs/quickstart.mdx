...
# Use Postgres as the primary data store for observability (instead of ClickHouse)
backend = "postgres"
...
3. Download the following sample `docker-compose.yml` and `Dockerfile.postgres` files.
   This Docker Compose configuration sets up a development Postgres database (where TensorZero stores data), the TensorZero Gateway, and the TensorZero UI.

```bash
curl -LO "https://raw.githubusercontent.com/tensorzero/tensorzero/refs/heads/main/examples/quickstart/docker-compose.yml"
curl -LO "https://raw.githubusercontent.com/tensorzero/tensorzero/refs/heads/main/examples/quickstart/Dockerfile.postgres"
```
...
services:
  ...
  # Run Postgres migrations before starting the gateway.
...
```dockerfile title="Dockerfile.postgres"
FROM pgvector/pgvector:pg17-trixie

# Install pg_cron (required by TensorZero for scheduled maintenance tasks)
RUN apt-get update \
...
You can use TensorZero in Python with the OpenAI Python client.
