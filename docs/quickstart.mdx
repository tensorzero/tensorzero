...
- config/
  - tensorzero.toml
- after.py (or after.ts) see below
- before.py
- docker-compose.yml
```
...
The TensorZero Gateway is compatible with the **OpenAI SDK**, so you can use it with any language that has an OpenAI client library.
The only code changes are pointing the client at the gateway and using a TensorZero function name as the model.

<Tabs>
<Tab title="Python">

```python title="after.py" "base_url="http://localhost:3000/openai/v1"" "tensorzero::function_name::generate_haiku"
from openai import OpenAI
...
```ts title="after.ts" "baseURL: "http://localhost:3000/openai/v1"" "tensorzero::function_name::generate_haiku"
