...
output_type = "boolean"
optimize = "min"
description = "Evaluate whether any named entities are repeated, either within the same category or across different categories. Scores: True (entity repeated), False (no repetition)."
...
A [complete example](https://github.com/tensorzero/tensorzero/tree/main/examples/docs/guides/optimization/gepa/) of this workflow is available on GitHub.
...
```python
from tensorzero import InferenceFilterFloatMetric

metric_filter = InferenceFilterFloatMetric(
    metric_name="jaccard_similarity",
    value=0.8,
    comparison_operator=">=",
)
