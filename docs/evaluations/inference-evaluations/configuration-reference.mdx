...
```toml mark="email-guardrails" title="tensorzero.toml"
...
An optional description of the evaluation.
It helps TensorZero Autopilot understand the intent behind the evaluation.
...
```toml mark="draft-email" title="tensorzero.toml"
[evaluations.email-guardrails]
# ...
...
The `exact_match` and `llm_judge` evaluator types support an optional `cutoff` field.
It sets a threshold at which the test is considered passing, which is useful when evaluations are run as an automated test.
...
```text title="evaluations/email-guardrails/check-signature/claude_sonnet_4_5/system_instructions.txt"
