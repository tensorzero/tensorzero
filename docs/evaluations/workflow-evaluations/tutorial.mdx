...
Here, we'll walk through a stylized RAG workflow to illustrate the process of setting up and running a workflow evaluation, but the same process can be applied to any complex workflow.
...
For a more complex, runnable example, see the [Workflow Evaluations for Agentic RAG Example on GitHub](https://github.com/tensorzero/tensorzero/tree/main/examples/workflow_evaluations/simple-agentic-rag).
...
Each individual task corresponds to an _episode_, and the collection of these episodes is a _workflow evaluation run_.
...
Now you can start a workflow evaluation run.

During a workflow evaluation run, you specify which variants you want to pin during the run (i.e. the set of variants you want to evaluate).
...
The TensorZero client automatically tags your workflow evaluation runs with information about your Git repository if available (e.g. branch name, commit hash).
This metadata is displayed in the TensorZero UI so that you have a record of the code that was used to run the workflow evaluation.
We recommend that you commit your changes before running a workflow evaluation so that the Git state is accurately captured.
...
For each task we want to include in our workflow evaluation run, we need to start an episode.
...
To initialize an episode, you need to provide the `run_id` of the workflow evaluation run you want to include the episode in.
...
```python
episode_info = t0.workflow_evaluation_run_episode(
...
```bash
curl -X POST http://localhost:3000/workflow_evaluation_run/{run_id}/episode \
  -H "Content-Type: application/json" \
  -d '{
    "task_name": "user_query_123"
  }'
...
You can use the OpenAI SDK for inference calls by pointing it at the TensorZero Gateway.
Pass the `episode_id` from the workflow evaluation run episode via `extra_body` so that inferences are associated with the evaluation run.
...
generate_database_query_response = openai_client.chat.completions.create(
    model="tensorzero::function_name::generate_database_query",
    messages=[...],
    extra_body={"tensorzero::episode_id": str(episode_info.episode_id)},
)

search_result = my_blackbox_search_function(generate_database_query_response)

generate_final_answer_response = openai_client.chat.completions.create(
    model="tensorzero::function_name::generate_final_answer",
    messages=[...],
    extra_body={"tensorzero::episode_id": str(episode_info.episode_id)},
)
...
openai_client = AsyncOpenAI(base_url="http://localhost:3000/openai/v1", api_key="not-used")

generate_database_query_response = await openai_client.chat.completions.create(
    model="tensorzero::function_name::generate_database_query",
    messages=[...],
    extra_body={"tensorzero::episode_id": str(episode_info.episode_id)},
)
...
generate_final_answer_response = await openai_client.chat.completions.create(
    model="tensorzero::function_name::generate_final_answer",
    messages=[...],
    extra_body={"tensorzero::episode_id": str(episode_info.episode_id)},
)
...
Once you finish running all the relevant episodes for your workflow evaluation run, you can visualize the results in the TensorZero UI.
