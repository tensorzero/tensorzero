...
Most notably, you need to deploy Postgres to [enforce custom rate limits](/operations/enforce-custom-rate-limits), [run automated A/B tests](/experimentation/run-automated-ab-tests), and [set up auth for TensorZero](/operations/set-up-auth-for-tensorzero).
