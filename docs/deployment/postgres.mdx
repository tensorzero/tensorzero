...
When applying Postgres migrations, the gateway also attempts to apply the optimization schema. Failures are logged as warnings, and do not affect normal usage if you do not use `experimental_dynamic_in_context_learning` variants.
