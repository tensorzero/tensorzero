...
The TensorZero UI exposes the `/health` endpoint for health checks.
This endpoint checks that the UI is running, the associated configuration is valid, and the ClickHouse connection is healthy.
