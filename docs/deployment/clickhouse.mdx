...
It currently does not support sharded deployments.
...
By default, the TensorZero Gateway applies database migrations automatically when it starts up.
This behavior can be suppressed by setting `gateway.observability.disable_automatic_migrations = true` in `tensorzero.toml`. See XXX.
If automatic migrations are disabled, then you must apply them manually with `gateway --run-migrations-only` or by using your own code.
