...
- **Clustering** (`TENSORZERO_CLICKHOUSE_CLUSTER_NAME`): Enables replication across multiple ClickHouse nodes for high availability
- **Sharding** (`TENSORZERO_CLICKHOUSE_SHARDING_ENABLED`): Distributes data across multiple nodes for horizontal scaling

You can have clustering without sharding (replication only) or clustering with sharding (replication + data distribution).

#### Sharding configuration

When both clustering and sharding are enabled, TensorZero uses table-specific sharding keys optimized for each table's access patterns:

- Data distribution is handled automatically based on table content
- Each table uses a carefully designed sharding key for optimal performance
