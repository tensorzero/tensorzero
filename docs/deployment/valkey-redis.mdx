...
### Dedicated Valkey Instance for Rate Limiting (Optional)

By default, TensorZero uses a single Valkey instance for both **caching** and **rate limiting**.
...
Set `TENSORZERO_VALKEY_RATE_LIMITING_URL` to point rate limiting at a dedicated Valkey instance:

```bash title=".env"
# Shared instance for caching
TENSORZERO_VALKEY_URL="redis://valkey-cache:6379"

# Dedicated instance for rate limiting
TENSORZERO_VALKEY_RATE_LIMITING_URL="redis://valkey-ratelimit:6379"
```

If `TENSORZERO_VALKEY_RATE_LIMITING_URL` is not set, rate limiting uses the `TENSORZERO_VALKEY_URL` instance.

This separation lets you configure different eviction policies per instance:

- **Rate limiting instance**: Use `noeviction` policy to ensure rate limiting keys are never silently evicted.
- **Caching instance**: Use `allkeys-lru` or `volatile-ttl` policy for efficient cache management.
...
For stronger guarantees, use a [dedicated Valkey instance for rate limiting](#dedicated-valkey-instance-for-rate-limiting-optional) with `noeviction` policy.
