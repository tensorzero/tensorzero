...
When using the OpenAI Node SDK with the TensorZero OpenAI-compatible endpoint, you can pass custom attributes using the `headers` option in the second parameter.
You must prefix header names with `tensorzero-otlp-traces-extra-attribute-`:
...
When using the OpenAI Node SDK with the TensorZero OpenAI-compatible endpoint, you can pass custom resources using the `headers` option in the second parameter.
You must prefix header names with `tensorzero-otlp-traces-extra-resource-`:
