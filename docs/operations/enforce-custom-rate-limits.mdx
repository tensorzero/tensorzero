...
- **Resources:** Define what you're limiting (like model inferences, tokens, or cost) and the time window (per second, hour, day, week, or month). For example, "1000 model inferences per day", "500,000 tokens per hour", or "$10 per day".
...
- `model_inferences`: The number of model inferences made.
- `tokens`: The number of tokens consumed. You must specify `max_tokens` for a request if a token limit applies to it.
- `cost`: The cost in dollars. Cost values are specified as floating-point numbers (e.g. `10.0` for $10.00, `0.50` for $0.50).
...
For cost rate limits, the gateway estimates the cost of each request upfront using a configurable `default_cost` (defaults to $1.00).
After the request completes, the gateway adjusts the consumed budget based on the actual cost reported by the provider.
If your model providers don't report cost information, the default estimate is used without adjustment.
You should only use cost-based rate limiting if you've configured cost for the relevant model providers.
...
<Step title="Set up a rate limiting backend">
