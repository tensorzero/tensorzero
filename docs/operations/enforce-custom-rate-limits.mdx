...
Rate limiting requires either Valkey or Postgres as a backend. Valkey is recommended for high-throughput deployments because it performs better under high contention.

**Option A: Valkey (recommended)**

Set the `TENSORZERO_VALKEY_URL` environment variable:

```bash
export TENSORZERO_VALKEY_URL="redis://localhost:6379"
```

See the [Deploy Valkey](/deployment/valkey) guide for detailed instructions.

**Option B: Postgres**

If Valkey is not configured, TensorZero will use Postgres for rate limiting. See the [Deploy Postgres](/deployment/postgres) guide for instructions.
