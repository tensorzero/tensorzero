...
You can browse usage data for individual inferences as well as aggregated usage statistics in the TensorZero UI.
...
You can use `pointer_nonstreaming` and `pointer_streaming` instead of `pointer` if the usage format is different for streaming inferences.
...
You can also browse cost data for individual inferences as well as aggregated cost statistics in the TensorZero UI.
