...
This enables you to enforce organization-wide controls (e.g. [auth](/operations/set-up-auth-for-tensorzero), [rate limits](/operations/centralize-auth-rate-limits-and-more), [credentials](/operations/manage-credentials)) without restricting how teams build their LLM features.
...
This guide shows you how to set up a two-tier TensorZero Gateway deployment that manages credentials and enforces auth in the relay.
...
You can configure [auth](/operations/set-up-auth-for-tensorzero), [rate limits](/operations/centralize-auth-rate-limits-and-more), [credentials](/operations/manage-credentials), and other organization-wide controls in the relay gateway. See below for an example that enforces auth on the relay.
...
For example, let's configure the gateway to look for the API key in the `TENSORZERO_RELAY_GATEWAY_API_KEY` environment variable:

```toml title="edge-config/tensorzero.toml"
[gateway.relay]
# ...
api_key_location = "env::TENSORZERO_RELAY_GATEWAY_API_KEY"
# ...
```
...
```bash
TENSORZERO_RELAY_GATEWAY_API_KEY="sk-t0-..."
```
