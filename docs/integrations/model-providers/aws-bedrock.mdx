...
TensorZero supports many AWS authentication methods for AWS Bedrock.

TensorZero will attempt the following authentication methods in order:
...
TensorZero will attempt to automatically determine the AWS region using the AWS SDK.
Alternatively, you can also configure an explicit region in your provider's configuration (e.g. `region = "us-east-1"`).
