...
You must set the `AZURE_API_KEY` environment variable before running the gateway. (Note: `AZURE_OPENAI_API_KEY` is deprecated but still supported as a fallback.)
