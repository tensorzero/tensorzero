...
Additionally, you can set the model in the inference request to use a specific Groq model, without having to configure a function and variant in TensorZero.
...
For reasoning models (e.g. `qwen/qwen3-32b`), set `reasoning_format = "parsed"` to receive structured reasoning content separately from the main response.
