...
Additionally, you can set the model in the inference request to use a specific Mistral model, without having to configure a function and variant in TensorZero.
...
When `prompt_mode` is set, reasoning output from the model will be returned as thought blocks in the response.
