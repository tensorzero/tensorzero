...
Additionally, you can set the model in the inference request to use a specific Together AI model, without having to configure a function and variant in TensorZero.
...
You can generate embeddings from Together AI using the [OpenAI-compatible provider](/integrations/model-providers/openai-compatible). For example:

```toml title="tensorzero.toml" {5-7}
[embedding_models.multilingual-e5]
routing = ["together"]

[embedding_models.multilingual-e5.providers.together]
type = "openai"
