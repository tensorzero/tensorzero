...
GEPA is guided by evaluator scores, so let's define an [Inference Evaluation](/evaluations/inference-evaluations/tutorial) in your TensorZero configuration.
...
[functions.extract_entities.evaluators.judge_improvement.variants.baseline]
type = "chat_completion"
model = "openai::gpt-5-mini"
system_instructions = "functions/extract_entities/evaluators/judge_improvement/system_instructions.txt"
json_mode = "strict"
```
...
Launch GEPA by specifying the name of your function, dataset, and evaluators.
...
<ParamField body="evaluators" type="list[string]">
  List of evaluator names (defined under the function's configuration) used to
  score candidate variants. Mutually exclusive with `evaluation_name`; exactly
...
<ParamField body="evaluation_name" type="string">
  ...
  `evaluators`; exactly one must be provided.
</ParamField>
