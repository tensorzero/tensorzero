...
TensorZero automatically collects structured data about those inferences.

You can populate a named [dataset](/gateway/api-reference/datasets-datapoints) from your historical inferences using `create_datapoints_from_inferences`:
...
The GEPA API requires the gateway to be configured with [Postgres](/deployment/postgresql) for durable task execution.
