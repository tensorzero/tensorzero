...
After deploying the [TensorZero Gateway](/deployment/tensorzero-gateway) with [Postgres](/deployment/postgres), [build a dataset](/gateway/api-reference/datasets-datapoints) for the `extract_entities` function you configured.
You can create datapoints from historical inferences or external/synthetic datasets.

When you launch GEPA with a single `dataset_name`, the dataset is automatically split 50/50 into training and validation sets.
...
```bash
curl -X POST http://localhost:3000/v1/optimization/gepa \
...
The GEPA API requires the gateway to be configured with [Postgres](/deployment/postgres) for durable task execution.
...
```bash
curl http://localhost:3000/v1/optimization/gepa/{task_id}
```
...
The `experimental_launch_optimization` and `experimental_poll_optimization` methods with `GEPAConfig` still work but are deprecated.
We recommend using the dedicated GEPA API (`t0.optimization.gepa.launch` / `t0.optimization.gepa.get`) for new integrations.
...
<ParamField body="analysis_model" type="string" required>
  Model used to analyze inference results (e.g. `"openai::gpt-5.2"`).
</ParamField>

<ParamField body="mutation_model" type="string" required>
  Model used to generate prompt mutations (e.g. `"openai::gpt-5.2"`).
</ParamField>

<ParamField body="max_iterations" type="integer" required>
...
<ParamField body="train_dataset_name" type="string">
  Training dataset name. Must be paired with `val_dataset_name`. Mutually
  exclusive with `dataset_name`.
</ParamField>

<ParamField body="val_dataset_name" type="string">
  Validation dataset name. Must be paired with `train_dataset_name`. Mutually
  exclusive with `dataset_name`.
</ParamField>

{/* Evaluation Parameters */}

<ParamField body="evaluation_name" type="string">
  Name of a configured evaluation to use for scoring. Mutually exclusive with
  `evaluators`.
</ParamField>

<ParamField body="evaluators" type="list[string]">
  Inline list of top-level evaluator names. Mutually exclusive with
  `evaluation_name`.
</ParamField>

{/* Optional Parameters */}

<ParamField body="initial_variants" type="list[string]">
  List of variant names to initialize GEPA with. If not specified, uses all
  variants defined for the function.
</ParamField>

<ParamField body="variant_prefix" type="string">
...
<ParamField body="max_concurrency" type="integer">
  Maximum number of concurrent inference calls. Default: 10.
</ParamField>

<ParamField body="max_datapoints" type="integer">
  Maximum number of datapoints to use from the dataset. If not specified, uses
  all datapoints.
</ParamField>

<ParamField body="include_inference_for_mutation" type="boolean">
...
The response is a tagged union on the `status` field:
...
## `GEPAConfig` (Deprecated)

<Warning>

`GEPAConfig` is used with the deprecated `experimental_launch_optimization` / `experimental_poll_optimization` methods.
For new integrations, use the dedicated API described above.
...
<ParamField body="analysis_model" type="str" required>
  Model used to analyze inference results (e.g.
  `"anthropic::claude-sonnet-4-5"`).
</ParamField>

<ParamField body="evaluation_name" type="str" required>
...
<ParamField body="mutation_model" type="str" required>
...
<ParamField body="batch_size" type="int" default={5}>
  Number of training samples to analyze per iteration.
</ParamField>

<ParamField body="include_inference_for_mutation" type="bool" default={true}>
  Whether to include inference input/output in the analysis passed to the
  mutation model. Useful for few-shot examples but can cause context overflow
  with long conversations or outputs.
</ParamField>

<ParamField body="initial_variants" type="list[str]">
  List of variant names to initialize GEPA with. If not specified, uses all
  variants defined for the function.
</ParamField>
...
<ParamField body="max_iterations" type="int" default={1}>
  Maximum number of optimization iterations.
</ParamField>
...
<ParamField body="retries" type="RetryConfig">
...
<ParamField body="variant_prefix" type="str">
  Prefix for naming newly generated variants.
</ParamField>
