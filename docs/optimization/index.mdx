...
As this dataset builds up, you can use these workflows to generate powerful variants for your functions.
...
RFT is particularly effective for tasks with clear evaluation criteria that can be expressed as graders (string matching, NLP metrics, LLM judges, or custom logic).

TensorZero has a reinforcement fine-tuning integration with OpenAI (reasoning models like o4-mini).
See the [Reinforcement Fine-Tuning Guide](/optimization/reinforcement-fine-tuning-rft) to learn more.

### Direct Preference Optimization (DPO)

Direct preference optimization (DPO) &mdash; also known as preference fine-tuning &mdash; fine-tunes an LLM on a dataset of preference pairs.
...
You can also create your own optimization workflows.
Put simply, an optimization workflow takes inference and feedback data collected by TensorZero and generates a new set of variants for your functions.
You should be able to use virtually any LLM engineering workflow with TensorZero, ranging from automated prompt engineering to advanced RLHF workflows.
