...
1. You [build a dataset](/gateway/api-reference/datasets-datapoints) of examples with inputs and reference outputs for your function.
...
- `{{item.messages}}` — The message history from the training data item (i.e. the input to the model).
...
After deploying the [TensorZero Gateway](/deployment/tensorzero-gateway), [build a dataset](/gateway/api-reference/datasets-datapoints) of examples for the `extract_entities` function you configured.
You can create datapoints from historical inferences or external/synthetic datasets.

```python
import json

from tensorzero import CreateDatapointRequestJson, JsonDatapointOutputUpdate
...
datapoints = [
    CreateDatapointRequestJson(
        ...
        output=JsonDatapointOutputUpdate(
            raw=json.dumps({"person": [...], "organization": [...], ...})
        ),
    ),
    # ... more datapoints
]
