...
We provide [complete code examples](https://github.com/tensorzero/tensorzero/tree/main/examples/docs/guides/gateway/call-an-llm) on Github.
...
See the [Integrations](./integrations/) page to learn how to set up credentials for other LLM providers.
...
See [Clients](./clients) for more details.
...
See the [Inference API Reference](./api-reference/inference) for more details on the request and response formats.
...
See the [Inference (OpenAI) API Reference](./api-reference/inference-openai-compatible) for more details on the request and response formats.
...
See the [TensorZero Gateway Deployment](./deployment) page for more details.
...
You can call the TensorZero Gateway directly over HTTP to call any LLM with a unified API.
