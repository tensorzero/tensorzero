...
You can restrict the set of tools that can be called at inference time by using the `allowed_tools` variant of the `tool_choice` parameter.
...
You can achieve this by setting `tool_choice` to `{"type": "allowed_tools", "allowed_tools": {"tools": ["get_temperature"], "mode": "auto"}}` in your inference request.

### Defining tools dynamically at inference time

You can define tools dynamically at inference time by using the `tools` property.
...
You can define a tool dynamically with the `tools` property.
This field accepts a list of objects following the standard OpenAI tool format.
...
- `{"type": "function", "function": {"name": "tool_name"}}`: The model should use a specific tool. The tool must be defined in the `tools` section of the configuration file or provided in `tools`.

The `tool_choice` parameter can be set either in your configuration file (using TOML syntax, e.g. `{ specific = "tool_name" }`) or directly in your inference request (using the JSON format shown above).
...
Custom tools are passed dynamically at inference time via `tools` with `type: "custom"`:
...
See the [API Reference](/gateway/api-reference/inference-openai-compatible/) for full documentation on custom tool formats including grammar-based constraints.
