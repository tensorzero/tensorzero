...
You can restrict the set of tools that can be called at inference time by using the `tool_choice` parameter with the `allowed_tools` type.
...
You can achieve this by setting `tool_choice={"type": "allowed_tools", "allowed_tools": {"tools": ["get_temperature"], "mode": "auto"}}` in your inference request.

### Defining tools dynamically at inference time

You can define tools dynamically at inference time by using the `tools` parameter.
...
The `tools` parameter accepts a list of tool objects following the OpenAI format.
See the [API Reference](/gateway/api-reference/inference-openai-compatible/#tools) for full documentation on the tool object structure.
...
- `{ specific = "tool_name" }`: The model should use a specific tool. The tool must be defined in the `tools` section of the configuration file or provided dynamically via the `tools` parameter.
...
You can also provide them dynamically at inference time via the `tensorzero::provider_tools` parameter in `extra_body`.
...
Custom tools are passed dynamically at inference time via the `tools` parameter with `type: "custom"`:
...
See the [API Reference](/gateway/api-reference/inference-openai-compatible/#tools) for full documentation on custom tool formats including grammar-based constraints.
