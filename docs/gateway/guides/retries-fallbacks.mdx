...
For example, the configuration below would instruct the gateway to try `gpt_5_mini` and `claude_haiku_4_5` first (with 90/10 sampling probability), and finally `grok_4` if all previous variants fail.
