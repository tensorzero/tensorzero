...
# 1. `models.gpt_5_mini.providers.openai`
# 2. `models.gpt_5_mini.providers.azure`
...
[models.gpt_5_mini.providers.azure]
type = "azure"
...
[functions.extract_data.variants.gpt_5_mini]
...
[functions.extract_data.variants.grok_4]
type = "chat_completion"
model = "xai::grok-4-0709"

[functions.extract_data.variants.gemini_2_5_flash]
type = "chat_completion"
model = "google_ai_studio_gemini::gemini-2.5-flash"
```

You can also use weights to control the sampling probabilities of candidate variants.
...
```python
while variants:
    # Sample according to experimentation config (static, adaptive, etc.)
...
- `timeouts.non_streaming.total_ms` — the total time allowed for a non-streaming request to complete.
- `timeouts.streaming.ttft_ms` — the time allowed to receive the first token (TTFT) in a streaming request.
- `timeouts.streaming.total_ms` — the total time allowed for the entire streaming request to complete (measured from request start). This is useful for bounding the total duration of a stream, even after the first token has arrived.

For example, the following configuration sets a 15-second timeout for non-streaming requests, a 3-second TTFT timeout, and a 60-second total timeout for streaming requests to a particular model provider.
