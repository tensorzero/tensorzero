...
If no experimentation configuration is specified, the gateway will sample between all variants uniformly.

You can customize the sampling behavior, including fallback-only variants, using the `experimentation` section.

For example, the configuration below would instruct the gateway to try `gpt_5_mini` and `claude_haiku_4_5` first (with 90/10 sampling probably), and finally `grok_4` if all previous variants fail.
...
type = "static_weights"
candidate_variants = {"gpt_4o_mini_api_key_A" = 0.5, "gpt_4o_mini_api_key_B" = 0.5}
```
