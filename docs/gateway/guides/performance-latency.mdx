...
It achieves &lt;1ms P99 latency overhead under extreme load (see [Benchmarks](/gateway/benchmarks/)).
...
- Enable `gateway.observability.async_writes` to offload the responsibility of writing inference responses to ClickHouse to a background task, instead of waiting for ClickHouse to return the inference response. [Learn more →](/gateway/configuration-reference/#observabilityasync_writes)
