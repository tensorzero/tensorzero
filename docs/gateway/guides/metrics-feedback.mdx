...
Feedback captures the downstream outcomes of your LLM application, and drive the [experimentation](/experimentation/run-adaptive-ab-tests) and [optimization](/optimization/) workflows in TensorZero.
...
with TensorZeroGateway.build_http(gateway_url="http://localhost:3000") as t0:
    feedback_response = t0.feedback(
        ...
        inference_id=inference_id,  # alternatively, you can assign feedback to an episode_id
...
```python
ChatCompletion(
    id='01920c75-d114-7aa1-aadb-26a31bb3c7a0',
    choices=[
        Choice(
            finish_reason='stop',
            index=0,
    ...
    episode_id='01920c75-cdcb-7fa3-bd69-fd28cf615f91',
)
...
This example collects metrics and demonstrations for an LLM-powered data extraction tool, which can be used for fine-tuning and other optimization workflows.
