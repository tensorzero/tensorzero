...
## Supplying Your Own Episode IDs

The gateway automatically generates episode IDs when you don't provide one.
However, if you must generate episode IDs in advance, you can generate and supply a UUIDv7 string as the episode ID.

<Warning>

The `uuid7` library in Python (available under `pip install uuid7`) is broken.
UUIDv7 strings generated by that library will cause `Invalid Episode ID: Timestamp is in the future` errors.

Instead, use the `tensorzero.util.uuid7` function provided by the TensorZero SDK.
