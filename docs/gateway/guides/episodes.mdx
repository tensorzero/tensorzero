...
The gateway accepts an optional `tensorzero::episode_id` field in inference requests (passed via `extra_body` when using the OpenAI SDK).
When you make the first inference request, you don't have to provide an episode ID.
...
The gateway will use the episode ID to associate the two inference requests together.
...
ChatCompletion(
    ...
    choices=[
        Choice(
            ...
            index=0,
            message=ChatCompletionMessage(
                content='This haiku captures the intricate and intimate relationship between technology and human consciousness. '
