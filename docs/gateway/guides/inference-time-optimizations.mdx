...
k = 5
cutoff = 0.5  # Optional: filter examples by cosine distance
...
- The optional `cutoff` parameter filters examples based on their cosine distance from the input, ensuring only highly relevant examples are included.
