...
- The `max_age_s` parameter applies to the retrieval of cached responses.
  ...
  When using Valkey, entries expire according to the configured TTL (`cache.valkey.ttl_s`). The default is 24h.
...
TensorZero can analyze your inference history to detect prompt caching opportunities and recommend where to place cache breakpoints.
See the [Prompt Caching Analysis](/optimization/prompt-caching-analysis) guide to learn more.
