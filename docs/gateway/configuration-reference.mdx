...
- `write_queue_capacity` (integer, optional): Maximum number of inferences each gateway instance can hold in memory per table type before dropping new rows to protect against out-of-memory crashes. When the queue is full, new rows are dropped and an error is logged. Increase this if you see `"batch channel full"` log messages during traffic spikes, and monitor your gateway instances' memory usage to determine whether your setting is appropriate (default: `10000`)
