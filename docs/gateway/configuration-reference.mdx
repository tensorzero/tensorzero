...
[functions.draft-email.experimentation]
# ...
fallback_variants = ["generic-prompt-v1"]
# ...
```
...
random sampling will begin. It is strongly recommended to set this value to at least 10 so that the feedback sample statistics can stabilize before they are used to guide the sampling probabilities.
...
The error tolerance. Must be a value in `(0, 1)`.
...
The sub-optimality tolerance. Must be nonnegative.
...
The frequency, in seconds, with which sampling probabilities are updated. Updating the
...
since at that point random sampling ceases and the winner variant is used exclusively.
