...
Defines whether to automatically detect the AWS region to use with the Bedrock API.
...
<Warning>
  Enabling this setting makes SDK initialization slow in non-AWS environments.
  We recommend specifying the `region` field explicitly instead.
</Warning>
...
This field can be either a string for a single location, or an object with `default` and `fallback` fields for fallback support.
...
- `sdk` - use AWS SDK auto-detection
- `none` - treat as unspecified (useful in fallback configurations)
...
# or: region = { default = "dynamic::bedrock-region", fallback = "env::AWS_REGION" }
# ...
```
...
Custom endpoint URL for the AWS Bedrock API. Useful for AWS PrivateLink, FIPS endpoints, or local testing (e.g., LocalStack).
...
# or: endpoint_url = { default = "dynamic::bedrock-endpoint", fallback = "env::BEDROCK_ENDPOINT_URL" }
```
...
# or: region = "sdk"  # auto-detect using AWS SDK
# or: region = { default = "dynamic::sagemaker-region", fallback = "env::AWS_REGION" }
# ...
```
...
Custom endpoint URL for the AWS SageMaker API. Useful for AWS PrivateLink, FIPS endpoints, or local testing (e.g., LocalStack).
...
# or: endpoint_url = { default = "dynamic::sagemaker-endpoint", fallback = "env::SAGEMAKER_ENDPOINT_URL" }
```
