...
PostgreSQL is required for certain TensorZero features including [Track-and-Stop experimentation](/experimentation/run-adaptive-ab-tests/) and [authentication](/operations/set-up-auth-for-tensorzero).
PostgreSQL can also be used for [rate limiting](/operations/enforce-custom-rate-limits/), though [Valkey](/deployment/valkey) is recommended for high-throughput deployments.
