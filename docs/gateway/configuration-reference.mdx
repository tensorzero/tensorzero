...
- `write_queue_capacity` (integer, optional): Capacity of the bounded input channel per table type. When the channel is full, new rows are dropped and logged instead of buffering without limit. Increase this value if you see `"Postgres batch channel full"` log messages during traffic spikes (default: `10000`)
