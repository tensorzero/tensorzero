...
by running `docker run --rm -e TENSORZERO_CLICKHOUSE_URL=$TENSORZERO_CLICKHOUSE_URL tensorzero/gateway:{version} --run-migrations-only`.
...
If all examples are filtered out due to the cutoff threshold, the variant falls back to default chat completion behavior.
