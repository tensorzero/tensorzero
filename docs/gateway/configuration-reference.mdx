...
An experiment is a procedure for randomly sampling function variants at inference time in order to learn about the relative
...
Note that if this section is present, it overrides any weights that are specified within individual function variant config
...
the variants in `candidate_variants` will be sampled according to provided weights, which must be nonnegative. (See the `candidate_variants`
...
the number of inferences required to draw conclusions from the experiment. See XXX for more info.
...
Plots based on the chosen metric are displayed in the `Experimentation` section of the `Functions` tab in the User Interface.
Additionally, if the experiment type is `track_and_stop`, then the chosen metric is used to dynamically update the sampling
probabilities for the variants in a way that is designed to quickly identify high performing variants.
...
A (possibly empty) set of function variants to use as fallback options. Each variant must be defined via `[functions.function_name.variants.variant_name]`
in the the `variants` sub-section (see above). If the set of `candidate_variants` is empty, or if the candidate variants
become unavailable for some reason, then variants will be sampled uniformly at random from `fallback_variants`.
...
The minimum number of samples per variant required before random sampling begins. Sampling from the `candidate_variants` will proceed
...
random sampling will begin, with the sampling probabilities determined by the experiment `type` (see above). For `type = "track_and_stop"`,
it is strongly recommended to set this value to at least 10 so that the feedback sample statistics can stabilize before they are used
...
The error tolerance when the experiment type is `track_and_stop`. Must be a value in `(0, 1)`.
When `type = "track_and_stop"`, the experimentation algorithm aims to identify a "winner" variant that has the
...
variant is not chosen as the winner at 5%. See XXX for more information.
...
The sub-optimality tolerance when the experiment type is `track_and_stop`. Must be nonnegative.
When `type = "track_and_stop"`, the experimentation algorithm aims to identify a "winner" variant whose
...
The frequency, in seconds, with which sampling probabilities are updated if `type = "track_and_stop"`. Updating the
...
quickly label the "winner" variant.
...
The minimum sampling probability for each candidate variant. This field is ignored if the experiment type is `uniform` or
`static_weights`. When `type = "track_and_stop"`, the aim of the experiment is to identify an epsilon-best variant, without
necessarily differentiating sub-optimal variants, so the primary use for this field is to enable the user
to learn about the performance of sub-optimal variants. Once `track_and_stop` picks a winner arm, random sampling ceases
and the winner variant is used exclusively, so this field is subsequently ignored.
