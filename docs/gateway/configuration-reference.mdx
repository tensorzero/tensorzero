...
in the the `variants` sub-section (see above). If inference fails with all of the
...
The minimum number of samples per variant required before random sampling begins. Must be >= 1. Sampling from the `candidate_variants` will proceed
...
The minimum sampling probability for each candidate variant. Must be nonnegative. Note that `min_prob * number_of_candidate_variants` must not exceed 1.0, since the minimum probabilities for all candidate variants must sum to at most 1.0.

The aim of a `track_and_stop` experiment is to identify
