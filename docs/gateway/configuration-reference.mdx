...
Note that if this section is present, it overrides any `weight` values that are specified within individual function variant config blocks.
...
The `uniform` type samples variants uniformly at random from all variants defined in the function.
...
If `null`, the gateway will connect to PostgreSQL if the `TENSORZERO_POSTGRES_URL` environment variable is set, otherwise it will disable PostgreSQL with a warning.
...
```toml title="tensorzero.toml"
[postgres]
enabled = true  # Require PostgreSQL connection
```
...
Rate limiting requires PostgreSQL. You must set the `TENSORZERO_POSTGRES_URL` environment variable and ensure `postgres.enabled` is not set to `false`.
