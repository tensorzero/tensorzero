...
If `true`, then the migrations are not run upon launch and must instead be run manually,
either by running `gateway --run_migrations_only` or via the user's own code.
