...
- We configured `observability.enabled = false` (i.e. disabled logging inferences to your database) in the TensorZero Gateway to make the scenarios comparable. (Even then, the observability features run asynchronously in the background, so they wouldn't materially affect latency given a powerful enough database deployment.)
