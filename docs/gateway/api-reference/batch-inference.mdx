...
- **Type:** list of inference response objects (see below)

Each object in the list corresponds to a single inference in the batch.
The format depends on the function type.

**Chat Function Response**

Each inference object has the following fields:
...
- `content` (list of content blocks): The content blocks generated by the model. A content block can have `type` equal to `text`, `tool_call`, or `thought`.
  - If `type` is `text`: `text` (string) contains the generated text.
  - If `type` is `tool_call`: `arguments` (object or null), `id` (string), `name` (string or null), `raw_arguments` (string), and `raw_name` (string).
  - If `type` is `thought`: `text` (string, optional) and `signature` (string, optional).
- `usage` (object, optional): `input_tokens` (integer), `output_tokens` (integer), and `cost` (number or null).
...
- `episode_id` (UUID): The ID of the episode associated with the inference.
- `variant_name` (string): The name of the variant used for the inference.
- `output` (object): Contains `raw` (string, the raw model output) and `parsed` (object or null, the parsed JSON output).
- `usage` (object, optional): `input_tokens` (integer), `output_tokens` (integer), and `cost` (number or null).
...
- **Type:** list containing a single inference response object

The list contains a single object for the requested inference.
The format is the same as documented in the `GET /batch_inference/:batch_id` [response above](#inferences).
