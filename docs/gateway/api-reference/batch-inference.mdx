...
- `model_name` and `provider_name` (optional): When both are specified, this content block will only be included in the model provider input for that specific model and provider combination.
  If not set, the content block is passed to all model providers.

For example, the following hypothetical unknown content block will send the `daydreaming` content block to inference requests targeting `your_model_name` with `your_provider_name`.
