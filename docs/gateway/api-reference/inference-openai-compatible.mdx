...
##### `tensorzero::cache_options.enabled`
...
Note: When using `tensorzero::dryrun=true`, the gateway never writes to the cache.

##### `tensorzero::cache_options.max_age_s`
...
This field allows for dynamic inference parameters, i.e. defining parameters at runtime.
...
In the OpenAI-compatible endpoint, if you parameters provided in the OpenAI interface itself take precedence over the parameters provided in this field.
...
See [Configuration Reference](/gateway/configuration-reference/#functionsfunction_namevariantsvariant_name) for more details on the parameters.

This field should be provided as an extra body parameter in the request body.

<Accordion title="Example">

For example, if you wanted to dynamically override the `temperature` parameter for `chat_completion` variants, you'd include the following in the request body:
...
A list of provider-specific tools to pass to the model provider at inference time.
This is useful for enabling provider-native tools like OpenAI's web search.

Each object in the array has the following fields:

- `scope` (optional): Specifies which model/provider this tool applies to. If not provided, the tool applies to all providers. The scope object can have:
  - `model_name`: The model name
  - `model_provider_name`: The provider name
- `tool`: A JSON object containing the provider-specific tool definition

This field should be provided as an extra body parameter in the request body.

<Accordion title="Example">

For example, to enable OpenAI's web search tool:
...
See the [Inference API Reference](/gateway/api-reference/inference/) for details on how to provide such content blocks as input.
