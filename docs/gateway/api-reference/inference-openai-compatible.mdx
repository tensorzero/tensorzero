...
description: API reference for the OpenAI-compatible endpoints.
---
...
- **`prompt_tokens_details`**: Optional breakdown of prompt tokens (object)
  - **`cached_tokens`**: Number of tokens served from cache (integer)
  - **`audio_tokens`**: Number of audio input tokens (integer)
- **`completion_tokens_details`**: Optional breakdown of completion tokens (object)
  - **`reasoning_tokens`**: Number of tokens used for reasoning (integer)
  - **`audio_tokens`**: Number of audio output tokens (integer)
  - **`accepted_prediction_tokens`**: Successfully predicted tokens (integer)
  - **`rejected_prediction_tokens`**: Failed prediction tokens (integer)

The detail fields are only included when supported by the underlying provider.
