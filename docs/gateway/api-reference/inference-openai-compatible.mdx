...
#### `tensorzero::include_original_response`

- **Type:** boolean
- **Required:** no

If `true`, the original response from the model provider will be included in the response in the `tensorzero_original_response` field as a string.

See `tensorzero_original_response` in the [response](#response) section for more details.
...
- **Type:** string (optional, only when `tensorzero::include_original_response` is `true`)

The original response from the model provider as a JSON string.
...
The original chunk from the model provider as a JSON string.
