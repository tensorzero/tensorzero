...
If `true`, the raw response from the model provider will be included in the response in the `tensorzero_raw_response` field as a string.
...
The raw response from the model provider as a JSON string.
...
The raw chunk from the model provider as a JSON string.
