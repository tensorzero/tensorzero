...
- `raw_usage` (optional, only when `include_raw_usage` is `true`): An array of raw provider-specific usage data. Each entry contains:
  ...
  - `usage` (optional): The raw usage object from the provider.
