...
- Optional: If either of the following is specified, the modification will only be applied to the specified variant or model provider. If neither is specified, the modification applies to all variants and providers.
  - `variant_name`: Targets a specific variant
  - `model_name` and `provider_name`: Targets a specific model and provider combination in your configuration (does not support shorthand names)
...
...overrides the request headers so that `Safety-Checks` is set to `off` for `my_variant`, and `Intelligence-Level: AGI` is applied globally to all variants and providers:
