...
See the [Call LLMs with image & file inputs](/gateway/call-llms-with-image-and-file-inputs/) guide for more details on how to use images in inference.
...
Selects a namespace-specific experimentation config for this request.
If the function has a matching namespace config, it will be used instead of the base experimentation config.
If no matching config exists, the base config is used as a fallback.
...
See [Scope experiments with namespaces](/experimentation/scope-experiments-with-namespaces) for a full guide.
...
See [Export OpenTelemetry traces](/operations/export-opentelemetry-traces) for more details and examples.
...
- `provider_cache_read_input_tokens` (optional): The number of input tokens served from the provider's prompt cache. Only present when the provider reports cache metrics.
- `provider_cache_write_input_tokens` (optional): The number of input tokens written to the provider's prompt cache. Only present when the provider reports cache metrics.
- `cost`: The cost in dollars for the inference (number or null). Set to `null` when cost is not configured for the model provider or the provider does not report the relevant information.

See [Track usage and cost](/operations/track-usage-and-cost) for more information.
...
```toml title="tensorzero.toml" mark="draft_email"
# ...
...
```toml title="tensorzero.toml" "weather_bot" /"(get_temperature)"/ /(get_temperature)]/
# ...
...
```toml title="tensorzero.toml" "weather_bot"
# ...
...
```toml title="tensorzero.toml" mark="draft_email" mark="temperature" mark="chat_completion"
# ...
[functions.draft_email]
type = "chat"
# ...
...
```toml title="tensorzero.toml" mark="extract_email"
# ...
