...
- `cost`: The cost in dollars for the inference (number, optional). Only present when cost is configured for the model provider and the provider reports the relevant information.
