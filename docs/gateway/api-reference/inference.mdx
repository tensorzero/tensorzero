...
If `true`, the raw response from the model will be included in the response in the `raw_response` field as a string.
...
The raw response from the model provider (only available when `include_raw_response` is `true`).
