...
- Optional: `variant_name` or `model_provider_name`: If specified, the modification will only be applied to the specified variant or model provider. If neither is specified, the modification applies to all variants and providers.
...
```json
{
  ...
  "extra_body": [
    ...
    {
      // No `variant_name` or `model_provider_name` specified, so it applies to all variants and providers
