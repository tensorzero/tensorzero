...
<Step title="Initialize the TensorZero Client">

Let's initialize the TensorZero client and point it to the gateway we just launched.
...
Both options support connecting to a standalone gateway with `build_http` or running the gateway embedded in your application with `build_embedded`.
