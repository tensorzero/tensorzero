...
| `input_tokens`        |   Nullable(UInt32)    | Total number of input tokens consumed                      |
| `output_tokens`       |   Nullable(UInt32)    | Total number of output tokens generated                    |
| `input_tokens_details` | Nullable(String)     | JSON object with input token breakdown                     |
| `output_tokens_details` | Nullable(String)    | JSON object with output token breakdown                    |
...
The `input_tokens_details` and `output_tokens_details` fields contain JSON objects with granular token breakdowns when supported by the provider:

**`input_tokens_details`:**
- `cached_tokens`: Number of tokens served from cache (OpenAI, Anthropic, Gemini, AWS Bedrock)
- `audio_tokens`: Number of audio input tokens (OpenAI multimodal models)

**`output_tokens_details`:**
- `reasoning_tokens`: Number of tokens used for reasoning/thinking (OpenAI o1/o3 models)
- `audio_tokens`: Number of audio output tokens (OpenAI multimodal models)
- `accepted_prediction_tokens`: Successfully predicted tokens from speculative decoding (OpenAI)
- `rejected_prediction_tokens`: Failed prediction tokens that were regenerated (OpenAI)
