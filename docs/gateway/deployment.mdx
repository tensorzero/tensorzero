...
See the <a href="https://clickhouse.com/en/install" target="_blank">ClickHouse documentation</a> for more details on configuring your ClickHouse deployment.
...
See the ClickHouse [replication documentation](https://clickhouse.com/en/engines/table-engines/mergetree-family/replication#converting-from-mergetree-to-replicatedmergetree) for more details.
...
- If you're using the GCP Vertex provider, you also need to mount the credentials for a service account in JWT form (described [here](https://cloud.google.com/authentication/provide-credentials-adc#service-account)) to `/app/gcp-credentials.json` using an additional `-v` flag.
