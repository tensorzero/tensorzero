...
fn build_get_model_inferences_query(inference_id: Uuid) -> QueryBuilder<sqlx::Postgres> {
    let mut qb = QueryBuilder::new(
        ...
        SELECT
            ...
            i.input_tokens,
            i.output_tokens,
            i.cache_read_input_tokens,
            i.cache_write_input_tokens,
            i.response_time_ms,
            i.model_name,
...
pub(super) fn build_insert_model_inferences_query(
    ...
    qb.push_values(rows.iter().zip(&timestamps), |mut b, (row, created_at)| {
        b.push_bind(row.id)
            ...
            .push_bind(row.cache_read_input_tokens.map(|v| v as i32))
            .push_bind(row.cache_write_input_tokens.map(|v| v as i32))
...
impl<'r> sqlx::FromRow<'r, sqlx::postgres::PgRow> for StoredModelInference {
    fn from_row(row: &'r sqlx::postgres::PgRow) -> Result<Self, sqlx::Error> {
        ...
        let cache_read_input_tokens: Option<i32> = row.try_get("cache_read_input_tokens")?;
        let cache_write_input_tokens: Option<i32> = row.try_get("cache_write_input_tokens")?;
        ...
        Ok(StoredModelInference {
            ...
            cache_read_input_tokens: cache_read_input_tokens.map(|v| v as u32),
            cache_write_input_tokens: cache_write_input_tokens.map(|v| v as u32),
