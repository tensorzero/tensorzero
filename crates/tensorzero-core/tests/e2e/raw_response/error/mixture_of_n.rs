//! E2E tests for `raw_response` in mixture-of-n candidate/fuser failure scenarios.
//!
//! Scenarios:
...
//! - Fuser failure: both candidates succeed, fuser errors → 200 OK with random selection + error entries
...
/// running the fuser. Expects: 1 error entry (failed candidate) + 1 success entry.
#[tokio::test]
async fn test_mixture_of_n_partial_fail_non_streaming() {
...
/// Mixture-of-N uses fake streaming. The surviving candidate IS the stream, so its
/// raw_response is not in a `raw_response` chunk — only the failed candidate's
/// error entry appears there.
#[tokio::test]
async fn test_mixture_of_n_partial_fail_streaming() {
    let episode_id = Uuid::now_v7();
    let payload = json!({
        "function_name": "mixture_of_n_raw_response_partial_fail",
        "variant_name": "mixture_of_n",
    ...
    assert!(
        !found_raw_chunk,
        "Mixture-of-N streaming should NOT have raw_chunk (fake streaming)"
    );
...
#[tokio::test]
async fn test_mixture_of_n_all_fail_non_streaming() {
    let episode_id = Uuid::now_v7();
    let payload = json!({
...
#[tokio::test]
async fn test_mixture_of_n_all_fail_streaming() {
    let episode_id = Uuid::now_v7();
    let payload = json!({
        "function_name": "mixture_of_n_raw_response_all_fail",
        "variant_name": "mixture_of_n",
...
// =============================================================================
// Fuser failure (both candidates succeed, fuser errors)
// =============================================================================

/// Non-streaming: both candidates succeed, fuser fails with raw_response.
/// Expects 200 OK (random selection) with 2 success entries + 1 error entry.
#[tokio::test]
async fn test_mixture_of_n_fuser_fail_non_streaming() {
    let episode_id = Uuid::now_v7();
    let payload = json!({
        "function_name": "mixture_of_n_raw_response_fuser_fail",
        "variant_name": "mixture_of_n",
        "episode_id": episode_id,
        "input": {
            "system": {"assistant_name": "AskJeeves"},
    ...
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "Response should be successful (fuser failed, random candidate selected)"
    );
    ...
    // 2 success entries (both candidates) + 1 error entry (fuser)
    assert_eq!(
        raw_response_array.len(),
        3,
        "Expected 3 raw_response entries (2 candidates + 1 failed fuser), got {}. Response: {response_text}",
        raw_response_array.len()
    );
    ...
    for entry in raw_response_array {
        if entry.get("model_inference_id").is_some() {
            ...
            assert_eq!(
                entry.get("provider_type").unwrap().as_str().unwrap(),
                "dummy",
                "Failed fuser provider_type should be `dummy`"
            );
            assert_eq!(
                entry.get("data").unwrap().as_str().unwrap(),
                "dummy error raw response",
                "Failed fuser data should be `dummy error raw response`"
            );
        }
    }
    assert_eq!(
    ...
    assert_eq!(error_count, 1, "Expected 1 error entry (failed fuser)");
}

/// Streaming: both candidates succeed, fuser fails.
/// Mixture-of-N uses fake streaming. The selected candidate IS the stream, so its
/// raw_response is not in a `raw_response` chunk. Only the non-selected candidate
/// and the failed fuser appear there.
#[tokio::test]
async fn test_mixture_of_n_fuser_fail_streaming() {
    let episode_id = Uuid::now_v7();
    let payload = json!({
        "function_name": "mixture_of_n_raw_response_fuser_fail",
        "variant_name": "mixture_of_n",
