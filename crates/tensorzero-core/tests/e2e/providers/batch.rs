...
fn expected_default_max_tokens(provider: &E2ETestProvider) -> u64 {
    if provider.model_name.starts_with("o1") {
        return 1000;
    }

    if provider.model_name.starts_with("gcp-gemini-2.5-") {
        return 200;
    }
...
pub async fn test_start_simple_image_batch_inference_request_with_provider(
    ...
    let expected_max_tokens = expected_default_max_tokens(&provider);
    assert_eq!(
        inference_params
            .get("max_tokens")
            .unwrap()
...
pub async fn test_parallel_tool_use_batch_inference_request_with_provider(
    ...
    let max_tokens = inference_params
        ...
        .as_u64()
        .unwrap();
    assert_eq!(max_tokens, expected_default_max_tokens(&provider));
