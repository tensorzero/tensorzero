...
fn expected_batch_max_tokens(provider: &E2ETestProvider) -> u64 {
    if provider.model_name.starts_with("o1") {
        return 1000;
    }

    if provider.variant_name.starts_with("gcp-vertex-gemini-flash") {
        return 200;
    }
...
pub async fn test_start_simple_image_batch_inference_request_with_provider(
    ...
    let expected_max_tokens = expected_batch_max_tokens(&provider);
    assert_eq!(
        inference_params
            .get("max_tokens")
            .unwrap()
