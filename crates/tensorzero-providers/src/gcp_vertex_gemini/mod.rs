...
pub struct GCPVertexGeminiSupervisedRow<'a> {
    ...
    /// `tensorzero-core`'s `FunctionTool` without lifetime gymnastics.
    /// They get re-projected as `GCPVertexGeminiSFTTool` at serialize time.
    #[serde(rename = "tools")]
...
fn serialize_tool_defs_as_sft_tools<S: serde::Serializer>(
    ...
    for def in defs {
        seq.serialize_element(&GCPVertexGeminiSFTTool::from(def))?;
    }
    seq.end()
}

impl<'a> GCPVertexGeminiSupervisedRow<'a> {
    /// Constructor for callers (e.g. core's optimizer) that already have the
    /// converted contents, system_instruction and tool definitions.
    pub fn new(
...
impl<'a> From<&'a FunctionToolDef> for GCPVertexGeminiSFTTool<'a> {
    fn from(tool: &'a FunctionToolDef) -> Self {
