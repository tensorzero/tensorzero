...
impl TypescriptJudgeExecutor {
    ...
    pub fn new(pool: RlmPool, ts_checker: Arc<TsCheckerPool>) -> anyhow::Result<Self> {
        let pool = pool
            ...
            .map_err(|e| anyhow::anyhow!("Failed to configure TypeScript judge heap limit: {e}"))?;
    ...
    pub async fn with_defaults() -> anyhow::Result<Self> {
        ...
        let ts_checker = Arc::new(
            TsCheckerPool::new(1)
                .await
                .map_err(|e| anyhow::anyhow!("Failed to build TsCheckerPool for TS judge: {e}"))?,
        );
...
pub async fn run_typescript_judge_evaluator(
    ...
    let exec_result =
        exec_result.map_err(|e| anyhow::anyhow!("TypeScript evaluator execution failed: {e}"))?;
...
mod tests {
    ...
    fn test_boolean_false_result() {
        let result =
            parse_evaluator_result(Some("false".to_string()), make_boolean_config().output_type);
        let value = result.expect("should succeed").expect("should have value");
