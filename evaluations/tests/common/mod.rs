...
use tensorzero::{
    Client, ClientBuilder, ClientBuilderMode, MaybeLegacyChatInferenceDatapoint,
    MaybeLegacyJsonInferenceDatapoint,
};
...
pub async fn write_chat_fixture_to_dataset(
    ...
    let mut datapoints: Vec<MaybeLegacyChatInferenceDatapoint> = Vec::new();
    // Iterate over the lines in the string
    for line in fixture.lines() {
        let mut datapoint: MaybeLegacyChatInferenceDatapoint = serde_json::from_str(line).unwrap();
...
pub async fn write_json_fixture_to_dataset(
    ...
    let fixture = std::fs::read_to_string(fixture_path).unwrap();
    let fixture = fixture.trim();
    let mut datapoints: Vec<MaybeLegacyJsonInferenceDatapoint> = Vec::new();
    // Iterate over the lines in the string
    for line in fixture.lines() {
        let mut datapoint: MaybeLegacyJsonInferenceDatapoint = serde_json::from_str(line).unwrap();
        datapoint.id = Uuid::now_v7();
        if let Some(dataset_name) = dataset_name_mapping.get(&datapoint.dataset_name) {
            datapoint.dataset_name = dataset_name.to_string();
        }
