...
mod tests {
    ...
    #[test]
    fn test_openrouter_response_with_metadata_try_into() {
        ...
        let openrouter_response_with_metadata = OpenRouterResponseWithMetadata {
            response: valid_response,
            raw_response: "test_response".to_string(),
            latency: Latency::NonStreaming {
                response_time: Duration::from_secs(0),
            },
            request: OpenRouterRequest::new("test_model", &generic_request).unwrap(),
            generic_request: &generic_request,
        };
        let inference_response: ProviderInferenceResponse =
            openrouter_response_with_metadata.try_into().unwrap();
